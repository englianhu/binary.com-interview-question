...
##
## 中科红旗（百家争鸣，文艺复兴）
## Oversea Greater Chinese Union 大秦子民公会（秦孝公 / 姜太公 --- 陈祯禄公爵 / 邱德拔公爵）
## 史无前例，一统天下
##
##
##
...
基准 <- 样本[年份 == 2018]$日期[1]
序列基准1 <- 样本[日期 == 基准][1]$序列 #序列 == 1122001
...
# 样本2018半年 <- readRDS(paste0(.蜀道, '文艺数据库/fx/USDJPY/样本2018半年.rds'))
rm(基准, 样本)

#基准 <- 样本2018半年[年份 == 2018]$日期[1] #序列 == 1122001
## 由于首一千多个闭市价都是一样汇价，故此得先过滤后，
##   由原本序列号1122001过滤为1124001开始，再测试。
# 样本2018半年 <- 样本2018半年[序列 >= 1124001]
# 基准 <- 样本2018半年[序列 == 1124001]$日期[1]
...
# 整顿数据(1200, 文件名 = paste0('季节性自回归自动化_差分阶数', .差分阶数, '_季节性差分阶数', .季节性差分阶数, '_zoo'), 是否移除文件夹 = '勾', 是否移除列表 = '勾')
        ...
        季回归2 <- llply(季节性与否, function(季节) {
          季回归3 <- llply(时间索引, function(时序) {
            成效 <- 日内高频季节性自回归(
              ...
              省略 = NULL, 计策谋略 = NULL, 季节性测试参数 = list(), 
              测试 = c('kpss', 'adf', 'pp'), 测试参数 = list(), 
              季节性测试 = c('seas', 'ocsb', 'hegy', 'ch'), 趋势 = NULL, 
              允许截距与否 = '勾', 允许包含均值与否 = '勾', 
              偏差调整与否 = '叉', 多管齐下与否 = '叉', 核心量 = 2, 
              包含均值与否 = '勾', 博克斯考克斯变换 = NULL)
          })
        })
      })
    })
...
# 整顿数据(1, 文件名 = '季节性自回归2018数据', 是否移除文件夹 = '勾')

### ============================= 模拟演算 01 ==================================
source('函数/日内高频季节性自回归自动化.R')

样本2018半年 <- readRDS(paste0(.蜀道, '文艺数据库/fx/USDJPY/样本2018半年.rds'))
## 由于首一千多个闭市价都是一样汇价，故此得先过滤后再测试。
时间索引 <- unique(样本2018半年$日期)
# 基准 <- 样本2018半年[年份 == 2018]$日期[1] #序列 == 1122001
## 由于首一千多个闭市价都是一样汇价，故此得先过滤后，
##   由原本序列号1122001过滤为1124001开始，再测试。
...
source('函数/日内高频季节性自回归自动化.R')
日内高频季节性自回归(
  时间索引 = 时间索引, 样本 = 样本2018半年, 数据量 = 数据量, 
  季节性与否 = 季节性与否, 静态与否 = 静态与否, 
  记载自回归与否 = 记载自回归与否, 逐步精化与否 = 逐步精化与否, 
  允许截距与否 = 允许截距与否, 允许均值与否 = 允许均值与否, 
  偏差调整与否 = 偏差调整与否, 多管齐下与否 = 多管齐下与否, 
  包含截距与否 = 包含截距与否)
...
计策谋略 <- ifelse2(计策谋略 == '冇', NULL, 计策谋略)
省略 <- ifelse2(省略 == '冇', NULL, 省略)
趋势 <- ifelse2(趋势 == '冇', NULL, 趋势)
允许截距与否 <- ifelse2(允许截距与否 == '勾', TRUE, FALSE)
允许均值与否 <- ifelse2(允许均值与否 == '勾', TRUE, FALSE)
相关系数 <- ifelse2(相关系数 == '冇', NULL, 相关系数)
...
## 随机选个迭代数测试。
迭数1 <- 迭代基准[1] #迭数1 <- 迭代基准[sample(迭代基准, 1)]
...
半成品 <- tryCatch({
  auto.arima(
    ...
    allowmean = 允许均值与否, lambda = 相关系数, 
    #lambda = 博克斯考克斯变换, 
    biasadj = 偏差调整与否, parallel = 多管齐下与否, num.cores = 核心量)
...
attr(半成品, '属性') <- list(
  差分阶数 = .差分阶数, 季节性差分阶数 = .季节性差分阶数, 
  静态与否 = 静态与否, 季节性与否 = 季节性与否, 
  信息量准则 = 信息量准则, 逐步精化与否 = 逐步精化与否, 
  逐步精化量 = 逐步精化量, 记载自回归与否 = 记载自回归与否, 
  计策谋略 = 计策谋略, 省略 = 省略, 趋势 = 趋势, 测试 = 测试, 
  季节性测试 = 季节性测试, 允许均值与否 = 允许均值与否, 
  相关系数 = 相关系数, 偏差调整与否 = 偏差调整与否, 
  数据量 = 数据量, 年月日时分 = 半成品$年月日时分)
cat('赋值完毕统计模型属性\n')

cat('开始命名统计模型\n')
模型名称 <- paste0(
  '自动季回归', '_数据量', 数据量, '_频率', 频率, 
  '_预测时间单位', 预测时间单位, '_', 半成品$年月日时分, 'CST.rds')
cat('命名统计模型完毕\n')
