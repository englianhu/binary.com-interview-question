...
l_ply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), p = 0, d = 1, q = 4, xreg = as.matrix(总汇[, .(", 参数, ")]))))"), .print = TRUE)

## forecast::auto.arima() 动态模型
## 如果eval()出错，可以使用dget()歼灭敌人巫贼巫婆非法回教徒公仆、任何不会中文或不是咱们世袭制道教徒华人或越族的人。
(奏章 <- llply(llply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), p = 0, d = 1, q = 4, xreg = as.matrix(总汇[, .(", 参数, ")]))))")), function(参数) eval(parse(text = 参数))) %>% 
    bind_rows(.id = '计数模型') %>% 
    data.frame() %>% 
    mutate(计数模型 = factor(
      c('外因周期自回归甲', '外因周期自回归乙', '外因周期自回归丙', 
...
```{r , error = TRUE}
summary(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), ic = 'aic', xreg = as.matrix(总汇[, .(时辰乙, 时分计)])))

summary(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), ic = 'aicc', xreg = as.matrix(总汇[, .(时辰乙, 时分计)])))

arimaorder(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), ic = 'aic', xreg = as.matrix(总汇[, .(时辰乙, 时分计)])))

arimaorder(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), ic = 'aicc', xreg = as.matrix(总汇[, .(时辰乙, 时分计)])))

## ic = 'aic'
(奏章 <- llply(llply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), ic = 'aic', xreg = as.matrix(总汇[, .(", 参数, ")]))))")), function(参数) eval(parse(text = 参数))) %>% 
    bind_rows(.id = '计数模型') %>% 
    data.frame() %>% 
    mutate(计数模型 = factor(
      c('外因周期自回归甲', '外因周期自回归乙', '外因周期自回归丙', 
...
## ic = 'aicc'
(奏章 <- llply(llply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(forecast::auto.arima(tk_xts(总汇[, .(年月日时分, 预测价)]), ic = 'aicc', xreg = as.matrix(总汇[, .(", 参数, ")]))))")), function(参数) eval(parse(text = 参数))) %>% 
    bind_rows(.id = '计数模型') %>% 
    data.frame() %>% 
    mutate(计数模型 = factor(
      c('外因周期自回归甲', '外因周期自回归乙', '外因周期自回归丙', 
...
## stats::arima() 静态模型
l_ply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(stats::arima(tk_xts(总汇[, .(年月日时分, 预测价)]), method = c('CSS-ML'), xreg = as.matrix(总汇[, .(", 参数, ")]), optim.method='Nelder-Mead')))"), .print = TRUE)

## stats::arima() 静态模型
(奏章 <- llply(llply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(stats::arima(tk_xts(总汇[, .(年月日时分, 预测价)]), method = c('CSS-ML'), xreg = as.matrix(总汇[, .(", 参数, ")]), optim.method='Nelder-Mead')))")), function(参数) eval(parse(text = 参数))) %>% 
    bind_rows(.id = '计数模型') %>% 
    data.frame() %>% 
    mutate(计数模型 = factor(
      c('外因周期自回归甲', '外因周期自回归乙', '外因周期自回归丙', 
...
## stats::arima() 动态模型
l_ply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(stats::arima(tk_xts(总汇[, .(年月日时分, 预测价)]), order = c(0, 1, 4), method = c('CSS-ML'), xreg = as.matrix(总汇[, .(", 参数, ")]), optim.method='Nelder-Mead')))"), .print = TRUE)
...
(奏章 <- llply(llply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(stats::arima(tk_xts(总汇[, .(年月日时分, 预测价)]), order = c(0, 1, 4), method = c('CSS-ML'), xreg = as.matrix(总汇[, .(", 参数, ")]), optim.method='Nelder-Mead')))")), function(参数) eval(parse(text = 参数))) %>% 
    bind_rows(.id = '计数模型') %>% 
    data.frame() %>% 
    mutate(计数模型 = factor(
      c('外因周期自回归甲', '外因周期自回归乙', '外因周期自回归丙', 
