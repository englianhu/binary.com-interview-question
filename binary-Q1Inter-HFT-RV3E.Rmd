...
subtitle: "[<span style='color:#DE5D83; background-color:black;'>金融衍生</span> - 筛选日内高频量化交易计数模型 <span style='color:#4E79A7'>**第三部乙（世袭制道教徒十二生肖秦人牧马，从满洲到星洲，戊戌变法歼灭所有敌人回教徒）**</span>](https://github.com/englianhu/binary.com-interview-question)"
...
<img src='诸子百家考工记/中国人后裔陈祯禄公爵，不忘初心方得始终.png' width = '400'>
*世袭制道教徒十二生肖，姓氏堂号子嗣贞节牌坊——从满洲到星洲*
...
- [Sarimax model issue with `xreg`](https://stackoverflow.com/a/76269638/3806250)

##### 动态模型

```{r stats::arima动态, error = TRUE}
## stats::arima() 动态模型
l_ply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(stats::arima(tk_xts(总汇[, .(年月日时分, 预测价)]), order = c(0, 1, 4), method = c('CSS-ML'), xreg = as.matrix(总汇[, .(", 参数, ")]))))"), .print = TRUE)
...
(奏章 <- llply(llply(list.apply(列表, paste, collapse = ', '), function(参数) paste0("as_tibble(accuracy(stats::arima(tk_xts(总汇[, .(年月日时分, 预测价)]), order = c(0, 1, 4), method = c('CSS-ML'), xreg = as.matrix(总汇[, .(", 参数, ")]))))")), function(参数) eval(parse(text = 参数))) %>% 
    bind_rows(.id = '计数模型') %>% 
    data.frame() %>% 
    mutate(计数模型 = factor(
      c('外因周期自回归甲', '外因周期自回归乙', '外因周期自回归丙', 
