...
牍 <- lapply(1:length(牍), function(迭甲) {
  ...
  matrix(ncol = 限甲, byrow = TRUE, dimnames = list(
    NULL, c('参数甲', '干支', '参数乙', '生肖', '参数丙', '朝代甲', '朝代乙', '朝代丙', '参数丁', 
            '在位皇帝甲', '在位皇帝乙', '参数戊', '年号甲', '年号乙')))
...
以上中国年号通史数据，採撷于[「词典网」首页>>年号](https://www.cidianwang.com/nianhao)。有关文本记载古称，请查阅[文本纪元](#文本纪元)，而“计数或机数，国之大事是也”请查阅[春秋战国名人](#春秋战国名人)。
...
```{r 天下简史, results = 'asis', error = TRUE}
## 秦国 China，秦人 Chinese
## 司马错得蜀既得楚
天下简史 %>% 
...
以上数据根据网上查询并手动添加。借鉴《春秋战国》、《太阁立志传》与《三国志》相关网游，它日有空再添加许多元素，包括钱谷、国内外经济贸易、习俗文化宗教语言、数术、人事变动、种族与宗教与收入行业等男女人口分布、学堂村垫、士农工商、军事分布、主要都城等。
...
- 文集建立日：农历二零二二年十二月廿三 壬寅年（虎年）癸丑月壬申日（阳历二零二三年一月十四日 / 民国一百一十一年十二月廿三）[^23]
...
[^23]: 欲知更多阳历/民国/农历日历详情，请登录中华民国农业部官网查询日历查询[阳历/农历/民国查询](https://ppg.naif.org.tw/naif/MarketInformation/Other/Calendar_V3.aspx)。
