...
<iframe width="400" height="225" src="https://www.youtube.com/embed/e7HCsYNMERE" title="「油管」「抖音」春风若有桃花意，落花时节又逢君" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

*出处：[「油管」「抖音」春风若有桃花意，落花时节又逢君](https://youtube.com/shorts/e7HCsYNMERE)*
...
if (!exists('.蜀道仓库')) .蜀道仓库 <- paste0(.蜀道, '诸子百家学府/世界文明史/')
        ...
        年份乙 = 年份 %>% 
            ...
            plyr::ldply(., function(迭) { 
                paste0(迭, collapse = '') %>% 
                    ...
                    cnum::num2c(lang = 'sc')}) %>% 
            unlist(), 
        年份 = if_else(substr(年份, 1, 3) == '公元前', paste0('公元前', 年份乙, '年'), 年份乙) %>% factor, 
        年份乙 = -cnum::c2num(年份乙))

周朝明细 <- str_split_fixed(周朝年号$明细, '，|：', 21) %>% 
  as_tibble()
## 周朝明细 <- 周朝明细[, colSums(is.na(周朝明细)) < nrow(周朝明细)]
周朝明细 <- 周朝明细[colSums((as.matrix(周朝明细 == ''))) < nrow(周朝明细)]
周朝明细 %<>% .[-c(1, 3, 5)]
names(周朝明细) <- c('干支', '生肖', '朝代甲', '朝代乙', '朝代丙', '在位皇帝甲', '在位皇帝乙', '在位皇帝丙', '年号甲', '年号乙', '年号丙')
周朝明细 %<>% 
    plyr::llply(str_replace_all, '在位皇帝|年号', '') %>% 
    data.frame() %>% 
    as_tibble()
...
周朝明细 %>% 
    dplyr::mutate(
        在位皇帝甲 = case_when(
            is.na(在位皇帝甲) & grepl('[^年]$', 在位皇帝乙) ~ 在位皇帝乙, 
            !is.na(在位皇帝甲) ~ 在位皇帝甲
            ), 
        在位皇帝乙 = case_when(
            is.na(在位皇帝乙) & !is.na(在位皇帝丙) & grepl('[^年]$', 在位皇帝丙) ~ 在位皇帝丙, 
            !is.na(在位皇帝乙) & grepl('[^年]$', 在位皇帝乙) & !is.na(在位皇帝丙) & grepl('[^年]$', 在位皇帝丙) & (在位皇帝乙 == 在位皇帝甲) ~ 在位皇帝丙, 
            !is.na(在位皇帝乙) ~ 在位皇帝乙
        ), 
        年号甲 = case_when(
            is.na(年号甲) & !is.na(在位皇帝丙) & grepl('[年]$', 在位皇帝丙) ~ 在位皇帝丙, 
            is.na(年号甲) & !is.na(年号乙) & grepl('[年]$', 年号乙) ~ 年号乙, 
            !is.na(年号甲) ~ 年号甲
        ), 
        年号乙 = case_when(
            !is.na(年号乙) & !is.na(年号甲) & grepl('[年]$', 年号甲) & (年号甲 == 年号乙) ~ 年号丙, 
            is.na(年号乙) & !is.na(年号丙) & grepl('[年]$', 年号丙) ~ 年号丙, 
            !is.na(年号乙) & !is.na(年号甲) & grepl('[年]$', 年号乙) & (年号甲 != 年号乙) ~ 年号乙, 
            !is.na(年号乙) ~ 年号乙
        ), 
        年号丙 = case_when(
            !is.na(年号丙) & !is.na(年号乙) & grepl('[年]$', 年号乙) & (年号丙 != 年号乙) ~ 年号丙, 
        ), 
        在位皇帝丙 = case_when(
            !is.na(在位皇帝丙) & !is.na(在位皇帝乙) & grepl('[^年]$', 在位皇帝丙) & (在位皇帝丙 != 在位皇帝乙) ~ 在位皇帝丙
        ))

周朝明细乙 <- 周朝明细 %>% 
    tidyr::unite('朝代丁', NA.:NA..1, remove = FALSE) %>% 
    tidyr::unite('朝代戊', NA..1:NA..2, remove = FALSE) %>% 
    tidyr::unite('在位皇帝甲', NA..2:NA..3, remove = FALSE) %>% 
    tidyr::unite('在位皇帝乙', NA..3:NA..4, remove = FALSE) %>% 
    tidyr::unite('在位皇帝丙', NA..4:NA..5, remove = FALSE) %>% 
    tidyr::unite('在位皇帝丁', NA..5:NA..6, remove = FALSE) %>% 
    tidyr::unite('在位皇帝戊', NA..6:NA..7, remove = FALSE) %>% 
    tidyr::unite('在位皇帝己', NA..7:NA..8, remove = FALSE) %>% 
    tidyr::unite('年号甲', NA..9:NA..10, remove = FALSE) %>% 
    tidyr::unite('年号乙', NA..10:NA..11, remove = FALSE) %>% 
    tidyr::unite('年号丙', NA..11:NA..12, remove = FALSE) %>% 
    mutate(
      '年号己' = NA..12, `NA.` = NULL, `NA..1` = NULL, `NA..2` = NULL, 
      `NA..3` = NULL, `NA..4` = NULL, `NA..5` = NULL, `NA..6` = NULL, 
      `NA..7` = NULL, `NA..8` = NULL, `NA..9` = NULL, `NA..10` = NULL, 
      `NA..11` = NULL, `NA..12` = NULL) %>% 
  plyr::llply(str_replace_all, '^_', '') %>% 
  data.frame() %>% 
  as_tibble()
...
周朝明细乙 %<>% 
    ...
    as_tibble() %>% 
    dplyr::mutate(
        年号己 = if_else((年号己 == '' | 年号己 == 年号丙), NA, 年号己), 
        年号丙 = if_else((年号丙 == '' | 年号丙 == 年号乙), NA, 年号丙), 
        年号乙 = if_else((年号乙 == '' | 年号乙 == 年号甲), NA, 年号乙), 
        年号甲 = if_else((年号甲 == '' | 年号甲 == 在位皇帝己), NA, 年号甲), 
        在位皇帝己 = if_else((在位皇帝己 == '' | 在位皇帝己 == 在位皇帝戊), NA, 在位皇帝己), 
        在位皇帝戊 = if_else((在位皇帝戊 == '' | 在位皇帝戊 == 在位皇帝丁), NA, 在位皇帝戊), 
        在位皇帝丁 = if_else((在位皇帝丁 == '' | 在位皇帝丁 == 在位皇帝丙), NA, 在位皇帝丁), 
        在位皇帝丙 = if_else((在位皇帝丙 == '' | 在位皇帝丙 == 在位皇帝乙), NA, 在位皇帝丙), 
        在位皇帝乙 = if_else((在位皇帝乙 == '' | 在位皇帝乙 == 在位皇帝甲), NA, 在位皇帝乙), 
        在位皇帝甲 = if_else((在位皇帝甲 == '' | 在位皇帝甲 == 朝代戊), NA, 在位皇帝甲), 
        朝代戊 = if_else((朝代戊 == '' | 朝代戊 == 朝代丁), NA, 朝代戊), 
        朝代丁 = if_else((朝代丁 == '' | 朝代丁 == 朝代丙), NA, 朝代丁), 
        朝代丙 = if_else((朝代丙 == '' | 朝代丙 == 朝代乙), NA, 朝代丙))
