...
经过筹算后比较**芳名榜（MASE）**如上所示，计数/机数模型**外因周期自回归戊**`的"as_tibble(accuracy(Arima(tk_xts(总汇[, .(年月日时分, 预测价)]), xreg = as.matrix(总汇[, .(时辰乙, 时分计)]))))"`是最优计数/机数模型。基于洋本数据观测值只有半年时间而且频率筛选为一的情况之下，`r ; nrow(总汇)`个观测值中的**廿四节气**参数有许多缺失值。

如果需要筹算更精准的算卜预测值，需要读取**Part I**中使用的原始数据已加以修饰并储存，**Part II**与**Part III**论文中三年七个月的数据，一共`r `样本 <- readRDS('/home/englianhu/文档/猫城/binary.com-interview-question-data/诸子百家学府/fx/USDJPY/样本乙.rds'); nrow(样本)`观测值。为了节省筹算时间，就只以半年数据为标准先筛选计数/机数模型。改日应用更高端计数/机数模型再筹算。
