...
author: "[®γσ, ξηg Lιαη Ημ](https://englianhu.github.io/) <img src='文艺坊图库/quantitative trader 1.png' height='14'> <img src='文艺坊图库/ENG.png' height='14'>® <img src='文艺坊图库/景知夏小姐.png' height='14'> <img src='文艺坊图库/Peking University 02.png' height='14'>" 
...
运算进度表 <- function(导入数据, 起点 = NULL, 秒计 = 60, .匹配元素 = '^sarima_', .周期性 = TRUE) {
  ...
  if (.周期性 == TRUE) {
    while(1) {
      ...
      布告栏 %<>% mutate(有无 = if_else(is.na(统计模型), 0, 1)) %>% 
		  spread(统计模型, 有无)
...
# --------- eval=FALSE ---------
时间索引 <- unique(样本$日期)
bse <- filter(样本, 年份 == 2016)$日期[1] #"2016-01-04" #1st trading date in 2nd year
时间索引 %<>% .[. >= bse]
时间索引 %<>% .[1:6] ##Take 6 dates as sample
#时间索引 %<>% .[. >= as_date('2016-01-04')]
数据量 <- 1200 #last 1200  observations 样本[(.N - (数据量 - 1)):.N]
预测时间单位 <- 1
...
## .order = c(0, 0, 0)
.order = permutations(6, 3, 0:5, repeats.allowed = TRUE) %>% 
  as_tibble %>% 
  rename(p = V1, d = V2, q = V3) %>% 
  filter(d <= 2)

## .seas = c(0, 0, 0)
.seas = permutations(6, 3, 0:5, repeats.allowed = TRUE) %>% 
  as_tibble %>% 
  rename(P = V1, D = V2, Q = V3) %>% 
  filter(D <= 1)
```
