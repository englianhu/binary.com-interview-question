...
- 外部因素週期性自回归综合滑均模型（ARIMAX）
- 季节性自回归综合滑均模型（SARIMA）
- 外部因素週期性季节性自回归综合滑均模型（SARIMAX）
- 外部因素週期性自回归分整综合滑均模型（Auto Regressive Fractionally Integrated Moving Average Exogenous - ARFIMAX）
...
### 季节性自回归综合滑均模型（ARIMAX）`ts()` & `auto.arima()`
...
llply(.模型选项, function(md) {
  ...
  if(md == 'auto') {
    ...
    srm0 <- llply(1:nrow(.order), function(迭数1) {
      srm1 <- llply(1:nrow(.seas), function(迭数2) {
        srm2 <- llply(时间索引, function(dte) {
          res <- 季节性自回归(
            时间索引 = dte, 样本, 
            数据量 = 数据量, 预测时间单位 = 预测时间单位, 
            .模型选项 = md, .order = unlist(.order[迭数1,]), 
            .seas = unlist(.seas[迭数2,]), 趋势 = NULL, 
...
### 外部因素週期性自回归综合滑均模型（Auto Regressive Fractionally Integrated Moving Average Exogenous - ARFIMAX）
