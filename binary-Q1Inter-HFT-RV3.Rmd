...
日内指数平滑数据结论 %>% 
  dplyr::mutate(
    mae = ifelse(
      rank(mae) <= 3, 
      cell_spec(
        paste0(round(mae, 30), ' (rank: ', sprintf('%1.f', rank(mae)), ')'), 
    ...
    mape = ifelse(
      rank(mape) <= 3, 
      cell_spec(
        paste0(round(mape, 30), ' (rank: ', sprintf('%1.f', rank(mape)), ')'), 
        color = 'darkgoldenrod', bold = TRUE), 
      cell_spec(
        paste0(round(mape, 30), ' (rank: ', sprintf('%1.f', rank(mape)), ')'), 
        color = 'grey', italic = TRUE)), 
    rmse = ifelse(
      rank(rmse) <= 3, 
      cell_spec(
        paste0(round(rmse, 30), ' (rank: ', sprintf('%1.f', rank(rmse)), ')'), 
        color = 'darkgoldenrod', bold = TRUE), 
      cell_spec(
        paste0(round(rmse, 30), ' (rank: ', sprintf('%1.f', rank(rmse)), ')'), 
        color = 'grey', italic = TRUE)), 
    ...
    mse = ifelse(
      rank(mse) <= 3, 
      cell_spec(
        paste0(round(mse, 30), ' (rank: ', sprintf('%1.f', rank(mse)), ')'), 
        color = 'darkgoldenrod', bold = TRUE), 
      cell_spec(
        paste0(round(mse, 30), ' (rank: ', sprintf('%1.f', rank(mse)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
