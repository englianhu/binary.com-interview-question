...
反攻巫裔回教徒，推翻马来Judi回教博彩庄Caspo Inc Cube Ltd回教九一一恐怖份子政府，建立六四学术中华政府
...
*数据：`r paste0(dim(日内指数平滑数据结论), collapse = ' x ')`*

以上图表显示，将再循环数据量参数设置为`频率 = 1`（**0.02小时一周期（1分钟循环1200次）**）的统计模型最为精准。
...
日内指数平滑数据总汇 <- fread(paste0(.蜀道仓库, '日内指数平滑数据总汇.csv'))
日内指数平滑数据总汇[, 年份 := data.table::year(年月日时分)]
日内指数平滑数据2016年总汇 <- 日内指数平滑数据总汇[年份 == 2016]
# saveRDS(日内指数平滑数据2016年总汇, paste0(.蜀道仓库, '日内指数平滑数据2016年总汇.rds'))
日内指数平滑数据2017年总汇 <- 日内指数平滑数据总汇[年份 == 2017]
# saveRDS(日内指数平滑数据2017年总汇, paste0(.蜀道仓库, '日内指数平滑数据2017年总汇.rds'))
日内指数平滑数据2018年总汇 <- 日内指数平滑数据总汇[年份 == 2018]
# saveRDS(日内指数平滑数据2018年总汇, paste0(.蜀道仓库, '日内指数平滑数据2018年总汇.rds'))
日内指数平滑数据2018年上半年总汇 <- 日内指数平滑数据2018年总汇[, 月份 := data.table::month(年月日时分)][月份 %chin% 1:6]
# saveRDS(日内指数平滑数据2018年上半年总汇, paste0(.蜀道仓库, '日内指数平滑数据2018年上半年总汇.rds'))

# 日内指数平滑数据2016年结论 <- 总汇结论(总汇 = 日内指数平滑数据2016年总汇, 文件名 = '日内指数平滑数据2016年', 是否储存结论 = '是')
日内指数平滑数据2016年结论 <- readRDS(paste0(.蜀道仓库, '日内指数平滑数据2016年结论.rds'))

# 日内指数平滑数据2017年结论 <- 总汇结论(总汇 = 日内指数平滑数据2017年总汇, 文件名 = '日内指数平滑数据2017年', 是否储存结论 = '是')
日内指数平滑数据2017年结论 <- readRDS(paste0(.蜀道仓库, '日内指数平滑数据2017年结论.rds'))

# 日内指数平滑数据2018年结论 <- 总汇结论(总汇 = 日内指数平滑数据2018年总汇, 文件名 = '日内指数平滑数据2018年', 是否储存结论 = '是')
日内指数平滑数据2018年结论 <- readRDS(paste0(.蜀道仓库, '日内指数平滑数据2018年结论.rds'))

# 日内指数平滑数据2018年上半年结论 <- 总汇结论(总汇 = 日内指数平滑数据2018年上半年总汇, 文件名 = '日内指数平滑数据2018年上半年', 是否储存结论 = '是')
日内指数平滑数据2018年上半年结论 <- readRDS(paste0(.蜀道仓库, '日内指数平滑数据2018年上半年结论.rds'))
```

### 总汇2016年结论

```{r 总汇2016年结论, results = 'asis'}
## 这儿将精准度调整至30位数，round(..., 30)
日内指数平滑数据2016年结论 %>% 
  dplyr::mutate(
    `均对误差（MAE）` = ifelse(
    ...
    `均方误差（MSE）` = ifelse(
      ...
      cell_spec(
        paste0(round(`均方误差（MSE）`, 30), ' (rank: ', sprintf('%1.f', rank(`均方误差（MSE）`)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
  kbl(caption = '最优统计模型（2016年总汇）', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
  #column_spec(2, background = '#556DAC') %>% 
...
*数据：`r paste0(dim(日内指数平滑数据2016年结论), collapse = ' x ')`*

### 总汇2017年结论

```{r 总汇2017年结论, results = 'asis'}
## 这儿将精准度调整至30位数，round(..., 30)
日内指数平滑数据2017年结论 %>% 
  dplyr::mutate(
    `均对误差（MAE）` = ifelse(
    ...
    `均方误差（MSE）` = ifelse(
      ...
      cell_spec(
        paste0(round(`均方误差（MSE）`, 30), ' (rank: ', sprintf('%1.f', rank(`均方误差（MSE）`)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
  kbl(caption = '最优统计模型（2017年总汇）', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
  #column_spec(2, background = '#556DAC') %>% 
...
*数据：`r paste0(dim(日内指数平滑数据2017年结论), collapse = ' x ')`*

### 总汇2018年（至7月7日）结论

```{r 总汇2018年结论, results = 'asis'}
## 这儿将精准度调整至30位数，round(..., 30)
日内指数平滑数据2018年结论 %>% 
  dplyr::mutate(
    `均对误差（MAE）` = ifelse(
    ...
    `均方误差（MSE）` = ifelse(
      ...
      cell_spec(
        paste0(round(`均方误差（MSE）`, 30), ' (rank: ', sprintf('%1.f', rank(`均方误差（MSE）`)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
  kbl(caption = '最优统计模型（2018年至7月7日总汇）', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
  #column_spec(2, background = '#556DAC') %>% 
...
*数据：`r paste0(dim(日内指数平滑数据2018年结论), collapse = ' x ')`*

### 总汇2018年上半年结论

```{r 总汇2018年上半年结论, results = 'asis'}
## 这儿将精准度调整至30位数，round(..., 30)
日内指数平滑数据2018年上半年结论 %>% 
  dplyr::mutate(
    `均对误差（MAE）` = ifelse(
    ...
    `均方误差（MSE）` = ifelse(
      ...
      cell_spec(
        paste0(round(`均方误差（MSE）`, 30), ' (rank: ', sprintf('%1.f', rank(`均方误差（MSE）`)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
  kbl(caption = '最优统计模型（2018年至上半年总汇）', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
  #column_spec(2, background = '#556DAC') %>% 
...
*数据：`r paste0(dim(日内指数平滑数据2018年上半年结论), collapse = ' x ')`*
...
> <span style='color:#9B0202; background-color:#252525;'>**《礼记·经解》孔子曰：**</span><span style='color:#FF0000; background-color:#000000;'>*「君子慎始，差若毫厘，谬以千里。」*</span>[^1]
...
由于敌国大象共和国黑客入侵，人为篡改数据导致**总汇2016年结论**和**总汇2017年结论**显示误差完全一样，*四个小数位数*的汇价的预测误差，有可能计算出*十五个小数位数*（倘若以秒计，飞秒（fs））误差值一模一样吗？上几周骇客频频入侵电子仪器`频率=12`的数据，骇客人为篡改为多一个观测值汇价。
...
- [「猫城」Confusing `POSIXlt` Warning !!!englianhu/binary.com-interview-question-data#4](https://github.com/englianhu/binary.com-interview-question-data/issues/4)
- **小插曲**：数据应该使用经过过滤`NA值`和重新赋值`周`、`周分计`、`日分计`、`时分计`、`序列`等参数和数据的**样本2**，而非**样本1**。不过`序列`参数的秩序顺序计算，及时有`NA值`都差异不大。
- 估计是由于骇客人为篡改数据，导致**总汇2016年结论**和**总汇2017年结论**显示误差完全一样，*四个小数位数*的汇价的预测误差，有可能计算出*十五个小数位数*（倘若以秒计，飞秒（fs））误差值一模一样吗？
- 上几周骇客频频入侵电子仪器将`频率=12`的数据，骇客人为篡改为多一个观测值汇价。
- `中科红旗`操作系统每天都被骇客入侵，前几周电脑时间都被篡改为`GMT+1`和这儿`GMT+8`迟了7个小时，在没留意时间的情况之下，继续运算，导致`频率=8`、`频率=6`、`频率=5`、`频率=4`、`频率=3`、`频率=2`、`频率=1`的数据时间有误，造成一些日期时间的汇价有误。然后一些缺失值再重新运算，会有些许差异。
- 得使用核武器毁灭马来巫裔回教911恐怖份子总警署、太子在野城（PutraJaya）、森美兰巫裔回教军事基地、巫裔回教皇宫，才能终止巫术屠杀，拯救全球。
