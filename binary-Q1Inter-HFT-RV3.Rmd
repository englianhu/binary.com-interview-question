  ...
  dplyr::mutate(
    ...
    均方根误差（RMSE） = ifelse(
      ...
      cell_spec(
        paste0(round(均方根误差（RMSE）, 30), ' (rank: ', sprintf('%1.f', rank(均方根误差（RMSE）)), ')'), 
        color = 'grey', italic = TRUE)), 
    s均对百分比误差（MAPE） = ifelse(
      rank(smape) <= 3, 
      cell_spec(
        paste0(round(smape, 30), ' (rank: ', sprintf('%1.f', rank(smape)), ')'), 
        color = 'darkgoldenrod', bold = TRUE), 
      cell_spec(
        paste0(round(smape, 30), ' (rank: ', sprintf('%1.f', rank(smape)), ')'), 
        color = 'grey', italic = TRUE)), 
...
*图表矩阵：`r paste0(dim(日内指数平滑数据结论), collapse = ' x ')`*

以上图表显示使用再循环数据量`频率 = 1`（**0.02小时一周期（1分钟循环1200次）**）最为精准。
...
*图表矩阵：`r paste0(dim(日内指数平滑数据2016年结论), collapse = ' x ')`*
...
*图表矩阵：`r paste0(dim(日内指数平滑数据2017年结论), collapse = ' x ')`*
...
*图表矩阵：`r paste0(dim(日内指数平滑数据2018年结论), collapse = ' x ')`*
...
*图表矩阵：`r paste0(dim(日内指数平滑数据2018年上半年结论), collapse = ' x ')`*
