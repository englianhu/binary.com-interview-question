...
.order = permutations(6, 3, 0:5, repeats.allowed = TRUE) %>% 
  as_tibble %>% 
  rename(自回归项数 = V1, 差分阶数 = V2, 移动平均项数 = V3) %>% 
  filter(差分阶数 <= 2)
...
```{r eval=FALSE}
# --------- eval=FALSE ---------
llply(.模型选项, function(md) {
  
  if(md == 'auto') {
    srm0 <- llply(.差分阶数, function(差分阶数) {
