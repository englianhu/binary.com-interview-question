...
author: "[®γσ, ξηg Lιαη Ημ](https://englianhu.github.io/) <img src='文艺坊图库/quantitative trader 1.png' height='14'> <img src='文艺坊图库/ENG.png' height='14'>® <img src='文艺坊图库/021 - 04.jpg' height='14'> <img src='文艺坊图库/Peking University 02.png' height='14'>" 
...
```{r 编织选项, class.source='萦回01', class.output='萦回02'}
## 更换时间区域，保留日期时间。
...
options(warn = -1, width = 999, knitr.table.format = 'html', 
        digits = 14, digits.secs = 6)

## https://stackoverflow.com/questions/39417003/long-vectors-not-supported-yet-abnor-in-rmd-but-not-in-r-script
## https://yihui.org/knitr/options
knitr::opts_chunk$set(
  class.source = '萦回01', class.output = '萦回02', class.error = '萦回03', 
...
names(样本) <- c('年月日时分', '年份', '季度', '月份', '周', '周日', '周分计', '日分计', '时分计', '序列', '日期', '闭市价')
...
## 倘若该路径尚无文件，储存文件。
if(!file.exists(paste0(.路径, 'data/fx/USDJPY/样本1.rds')) & exists('样本')) {
  saveRDS(样本, paste0(.路径, 'data/fx/USDJPY/样本1.rds'))}
```
...
以上图表显示数据日期，由`r paste(range(样本$日期), collapse = ' 至 ')`，而该数据也是在论文*(Part I)*、*(Part II)*中使用，并且相符。使用同样的数据，是为了在回测多元统计模型，才能获知并筛选最优统计模型。由于数据上的交易时间出现时差的缘故，在读取数据后就将数据上的时间更换，添加一小时时差为中国标准时间，以确保时间规律计算方面，不会出错。
