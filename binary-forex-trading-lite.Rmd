...
pkgs <- c('tint', 'devtools', 'lubridate', 'plyr', 'stringr', 'magrittr', 'dplyr', 'tidyr', 'turner', 'readr', 'tidyverse', 'quantmod', 'tidyquant', 'htmltools', 'highcharter', 'googleVis', 'formattable', 'DT', 'forecast', 'Mcomp', 'bsts', 'MCMCpack', 'PerformanceAnalytics', 'broom', 'microbenchmark', 'doParallel', 'Boruta', 'rugarch', 'parma', 'rmgarch', 'MASS', 'mvtnorm', 'tmvtnorm')
...
<center><iframe src="https://raw.githubusercontent.com/englianhu/binary.com-interview-question/ff20ee95aa60ef5cca3cf797066089103eb62acf/reference/quant-analyst-skills-test.pdf" width="auto" height="auto"></iframe></center>
...
tail(data.frame(varB1['Date'], varB1[grep('Bal', names(varB1))]))
...
```
  ...
  The 2nd staking method is based on real-time volativity which is the transaction will only stand if the highest or lowest price happenned within hte variance, same with the initial Kelly staking model.

```{r variance-staking2}
## Betting strategy 2 - Real range betting
HiLo <- function(txt) {
  eval(parse(text = paste0('ifelse(varHL2$USDJPY.High - varHL2$', txt, ' < 0, varHL2$USDJPY.Close - varHL2$', txt, ', varHL2$USDJPY.High - Hi(varHL2))')))
  }

varHi <- ifelse(varHL2$USDJPY.High - Hi(varHL2) < 0, 
                varHL2$USDJPY.Close - Hi(varHL2), 
                varHL2$USDJPY.High - Hi(varHL2))
                
varLo <- ifelse(Lo(varHL2) - varHL2$USDJPY.Low < 0, 
                Lo(varHL2) - varHL2$USDJPY.Close, 
                Lo(varHL2) - varHL2$USDJPY.Low)

varS <- ifelse(varHi >= 0, 1, 0)
varB <- ifelse(varLo >= 0, 1, 0)

varHi <- varHi * varS
varLo <- varLo * varB

varHi <- ifelse((varHi >= 0 & varLo == 0)|(varHi == 0 & varLo >= 0), 
                varHL2$USDJPY.High, 0)
varLo <- ifelse((varHi >= 0 & varLo == 0)|(varHi == 0 & varLo >= 0), 
                varHL2$USDJPY.Low, 0)

## Betting strategy 2 - Kelly Criterion
...
```{r variance-returns}
## focast the variance and convert to probability.
varB <- llply(fundList, function(dtx) mutate(dtx, ProbBVar = pnorm(Point.Forecast, mean = (USDJPY.High + USDJPY.Low)/2, sd = sd((USDJPY.High + USDJPY.Low)/2)), ProbSVar = 1 - ProbBVar) %>% tbl_df)

## Betting strategy 1 - Normal betting
