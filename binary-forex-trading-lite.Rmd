...
pkgs <- c('tint', 'devtools', 'lubridate', 'plyr', 'stringr', 'magrittr', 'dplyr', 'tidyr', 'readr', 'tidyverse', 'quantmod', 'tidyquant', 'htmltools', 'highcharter', 'formattable', 'DT', 'forecast', 'Mcomp', 'bsts', 'MCMCpack', 'PerformanceAnalytics', 'broom', 'microbenchmark', 'doParallel', 'Boruta', 'rugarch', 'parma', 'rmgarch', 'MASS', 'mvtnorm', 'tmvtnorm')
...
varHL <- fundList[grep('HiHi|LoLo', names(fundList))]
varHL1 <- suppressMessages(llply(varHL, function(dtx) {
  mm = tbl_df(dtx) %>% dplyr::select(Date, Point.Forecast, USDJPY.High, USDJPY.Low)
  names(mm)[2] = as.character(dtx$Model[1])
  names(mm) = str_replace_all(names(mm), 'HiHi', 'High')
  names(mm) = str_replace_all(names(mm), 'LoLo', 'Low')
  ntm = str_replace_all(names(mm), '.High|.Low', '') %>% unique %>% sort
  mm[ntm %in% names(mm),]
  }) %>% join_all) %>% tbl_df

ntm = str_replace_all(names(varHL1), '.High|.Low', '')[-1]
varHL2 <- suppressMessages(llply(ntm, function(nm) {
    ...
    data.frame(Date = varHL1['Date'], mld) %>% tbl_df
    }) %>% unique %>% join_all %>% tbl_df)

## Application of MASS::mvrnorm() or mvtnorm::rmvnorm()



varHL1 %>% gather(Model, Price, ANN.High:ZZZ.Low)

 %>% mutate(ProbBVar = pnorm(Point.Forecast, mean = (USDJPY.High + USDJPY.Low)/2, sd = sd((USDJPY.High + USDJPY.Low)/2)), ProbSVar = 1 - ProbBVar)
