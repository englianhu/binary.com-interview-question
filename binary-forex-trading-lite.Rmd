...
suppressAll(library('Mcomp'))
suppressAll(library('bsts'))
...
```{r stakingModels}
simStakes <- function(mbase, .prCat = 'Op', .baseDate = ymd('2015-01-01'), .parallel = FALSE, .setPrice = 'Cl', .initialFundSize = 1000, .fundLeverageLog = TRUE, .filterBets = FALSE) {
...
df.op <- cbind(df.op, forClose = df.op$Point.Forecast) %>% tbl_df
df.hi <- cbind(df.hi, forClose = df.hi$Point.Forecast) %>% tbl_df
df.mn <- cbind(df.mn, forClose = df.mn$Point.Forecast) %>% tbl_df
df.lo <- cbind(df.lo, forClose = df.lo$Point.Forecast) %>% tbl_df
df.cl <- cbind(df.cl, forClose = df.cl$Point.Forecast) %>% tbl_df
```

  Convert the probability based on closing price as the settled price. Here I try to set USD1 as the criteria (baseline) of daily sakes for every single match.

```{r prob-convertion, eval = FALSE}
##============================ EVAL = FALSE ================================
## convert all forecast price into probability.

## forecast the opened price.
df.op %<>% mutate(Prob = pnorm(forClose, mean = USDJPY.Close, sd = sd(USDJPY.Close)), 
                  stakes = 1)

## forecast the highest price.
df.hi %<>% mutate(Prob = pnorm(forClose, mean = USDJPY.Close, sd = sd(USDJPY.Close)), 
                  stakes = 1)

## forecast the mean price.
df.mn %<>% mutate(Prob = pnorm(forClose, mean = USDJPY.Close, sd = sd(USDJPY.Close)), 
                  stakes = 1)

## forecast the lowest price.
df.lo %<>% mutate(Prob = pnorm(forClose, mean = USDJPY.Close, sd = sd(USDJPY.Close)), 
                  stakes = 1)
...
```{r staking, eval = FALSE}
##============================ EVAL = FALSE ================================
## Kelly staking models.

## placed orders on the opened price.
df.op %<>% mutate(
      Diff = round(forClose - USDJPY.Close, 2), 
      Back = ifelse(forClose > USDJPY.Close, 1, 0), 
      Lay = ifelse(forClose < USDJPY.Close, 1, 0)) %>% 
    mutate(Return.Back = ifelse(Prob > 0.5, Diff * Back * stakes, 0), 
           Return.Lay = ifelse(Prob < 0.5, -Diff * Lay * stakes, 0))

## placed orders on  the highest price.
df.hi %<>% mutate(
      Diff = round(forClose - USDJPY.Close, 2), 
      Back = ifelse(forClose > USDJPY.Close, 1, 0), 
      Lay = ifelse(forClose < USDJPY.Close, 1, 0)) %>% 
...
## placed orders on  the mean price.
df.mn %<>% mutate(
      Diff = round(forClose - USDJPY.Close, 2), 
      Back = ifelse(forClose > USDJPY.Close, 1, 0), 
      Lay = ifelse(forClose < USDJPY.Close, 1, 0)) %>% 
...
## placed orders on  the lowest price.
df.lo %<>% mutate(
      Diff = round(forClose - USDJPY.Close, 2), 
      Back = ifelse(forClose > USDJPY.Close, 1, 0), 
      Lay = ifelse(forClose < USDJPY.Close, 1, 0)) %>% 
...
## placed orders on  the closed price.
df.cl %<>% mutate(
      Diff = round(forClose - USDJPY.Close, 2), 
      Back = ifelse(forClose > USDJPY.Close, 1, 0), 
      Lay = ifelse(forClose < USDJPY.Close, 1, 0)) %>% 
...
## A tibble: 20 × 5
#        .id    InitFund    Profit      LastFund       RR
#      <chr> <dbl>     <dbl>    <dbl>    <dbl>
#1  fundOPHI  1000 326.83685 1326.837 1.326837
#2  fundHIHI  1000   0.00000 1000.000 1.000000
#3  fundMNHI  1000 152.30210 1152.302 1.152302
#4  fundLOHI  1000 816.63808 1816.638 1.816638
#5  fundCLHI  1000 323.18564 1323.186 1.323186
#6  fundOPMN  1000 246.68001 1246.680 1.246680
#7  fundHIMN  1000 384.90915 1384.909 1.384909
#8  fundMNMN  1000   0.00000 1000.000 1.000000
#9  fundLOMN  1000 529.34170 1529.342 1.529342
#10 fundCLMN  1000 221.03926 1221.039 1.221039
#11 fundOPLO  1000 268.31155 1268.312 1.268312
#12 fundHILO  1000 649.35074 1649.351 1.649351
#13 fundMNLO  1000 298.28509 1298.285 1.298285
#14 fundLOLO  1000   0.00000 1000.000 1.000000
#15 fundCLLO  1000 208.85690 1208.857 1.208857
#16 fundOPCL  1000  30.55969 1030.560 1.030560
#17 fundHICL  1000 400.59057 1400.591 1.400591
#18 fundMNCL  1000 117.96808 1117.968 1.117968
#19 fundLOCL  1000 530.68975 1530.690 1.530690
#20 fundCLCL  1000   0.00000 1000.000 1.000000
## A tibble: 20 × 7
#        .id  StartDate LatestDate InitFund LatestFund    Profit        RR
#      <chr>     <date>     <date>    <dbl>      <dbl>     <dbl>     <dbl>
#1  fundOPHI 2015-01-02 2017-01-20 1000 201.699419 194.79166 29.198982
#2  fundHIHI 2015-01-02 2017-01-20 1000   6.907755   0.00000  1.000000
#3  fundMNHI 2015-01-02 2017-01-20 1000  75.593102  68.68535 10.943222
#4  fundLOHI 2015-01-02 2017-01-20 1000 592.614380 585.70662 85.789718
#5  fundCLHI 2015-01-02 2017-01-20 1000 199.023237 192.11548 28.811565
#6  fundOPMN 2015-01-02 2017-01-20 1000 145.334081 138.42633 21.039263
#7  fundHIMN 2015-01-02 2017-01-20 1000 245.812470 238.90472 35.585000
#8  fundMNMN 2015-01-02 2017-01-20 1000   6.907755   0.00000  1.000000
#9  fundLOMN 2015-01-02 2017-01-20 1000 359.728088 352.82033 52.075975
#10 fundCLMN 2015-01-02 2017-01-20 1000 127.528193 120.62044 18.461597
#11 fundOPLO 2015-01-02 2017-01-20 1000 159.124291 152.21654 23.035600
#12 fundHILO 2015-01-02 2017-01-20 1000 452.725480 445.81772 65.538726
#13 fundMNLO 2015-01-02 2017-01-20 1000 180.580704 173.67295 26.141734
#14 fundLOLO 2015-01-02 2017-01-20 1000   6.907755   0.00000  1.000000
#15 fundCLLO 2015-01-02 2017-01-20 1000 117.219609 110.31185 16.969276
#16 fundOPCL 2015-01-02 2017-01-20 1000  17.669553  10.76180  2.557930
#17 fundHICL 2015-01-02 2017-01-20 1000 256.110890 249.20313 37.075849
#18 fundMNCL 2015-01-02 2017-01-20 1000  57.745913  50.83816  8.359577
#19 fundLOCL 2015-01-02 2017-01-20 1000 357.560612 350.65286 51.762200
#20 fundCLCL 2015-01-02 2017-01-20 1000   6.907755   0.00000  1.000000
