...
## Plot the models
## opened price fit data
plot(forecast(ets(fit.op$Point.Forecast)))
forecast(ets(fit.op$Point.Forecast), h = 4)

## highest price fit data
plot(forecast(ets(fit.hi$Point.Forecast)))
forecast(ets(fit.hi$Point.Forecast), h = 4)

## mean price fit data (mean price of daily highest and lowest price)
plot(forecast(ets(fit.mn$Point.Forecast)))
forecast(ets(fit.mn$Point.Forecast), h = 4)

## lowest price fit data
plot(forecast(ets(fit.lo$Point.Forecast)))
forecast(ets(fit.lo$Point.Forecast), h = 4)

## opened price fit data
plot(forecast(ets(fit.cl$Point.Forecast)))
forecast(ets(fit.cl$Point.Forecast), h = 4)

fcdata <- do.call(cbind, list(USDJPY.FPOP = fit.op$Point.Forecast, USDJPY.FPHI = fit.hi$Point.Forecast, USDJPY.FPMN = fit.mn$Point.Forecast, USDJPY.FPLO = fit.lo$Point.Forecast, USDJPY.FPCL = fit.cl$Point.Forecast, USDJPY.Close = fit.op$USDJPY.Close))
fcdata <- na.omit(fcdata)
names(fcdata) <- c('USDJPY.FPOP', 'USDJPY.FPHI', 'USDJPY.FPMN', 'USDJPY.FPLO', 'USDJPY.FPCL', 'USDJPY.Close')
```
...
## Profit and Loss.
bind_cols(
  df.op %>% dplyr::select(Return.Back, Return.Lay) %>% colSums %>% data.frame, 
  df.hi %>% dplyr::select(Return.Back, Return.Lay) %>% colSums %>% data.frame, 
  df.mn %>% dplyr::select(Return.Back, Return.Lay) %>% colSums %>% data.frame, 
  df.lo %>% dplyr::select(Return.Back, Return.Lay) %>% colSums %>% data.frame, 
  df.cl %>% dplyr::select(Return.Back, Return.Lay) %>% colSums %>% data.frame)
```
