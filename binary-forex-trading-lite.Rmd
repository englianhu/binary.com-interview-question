...
simPrice <- function(mbase, .model = 'ZZZ', .prCat = 'Mn', .baseDate = ymd('2015-01-01'), .parallel = FALSE, .simulate = FALSE, .bootstrap = FALSE, .npaths = 5000) {
...
```{r simStakes}
simStakes <- function(mbase, .model = 'ZZZ', .prCat = 'Op', .baseDate = ymd('2015-01-01'), .parallel = FALSE, .setPrice = 'Cl', .initialFundSize = 1000, .fundLeverageLog = FALSE, .filterBets = FALSE, .simulate = FALSE, .bootstrap = FALSE, .npaths = 5000) {
  ...
  fit1 <- simPrice(mbase, .model = .model, .prCat = .prCat, .baseDate = .baseDate, .parallel = .parallel, .simulate = .simulate, .bootstrap = .bootstrap, .npaths = .npaths)
  fit1 <- data.frame(Date = index(fit1), coredata(fit1)) %>% tbl_df
  fit1 <- na.omit(fit1)
  
  ## forecast settlement price.
  fit2 <- simPrice(mbase, .model = .model, .prCat = .setPrice, .baseDate = .baseDate, .parallel = .parallel, .simulate = .simulate, .bootstrap = .bootstrap, .npaths = .npaths)
...
microbenchmark(res <- llply(ets.m, function(x) {
    llply(pp, function(y) {
        simStakes(mbase, .model = x, .prCat = y[1], .setPrice = y[2], .initialFundSize = 1000, .simulate = TRUE, .bootstrap = TRUE)
      cat(paste(x, ' with ', y, 'done', '\n'))
    }, .parallel = TRUE)
  cat(paste(x, 'done', '\n'))
}, .parallel = TRUE))
