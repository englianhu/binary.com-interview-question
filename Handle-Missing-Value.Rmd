...
subtitle: "[binary.com](https://github.com/englianhu/binary.com-interview-question) 面试试题 I - 数据缺失值管理"
...
以上比较均方误差，而边缘误差为`0.0000084`至`0.0000160`，故此弥补来的数据还是可行的。

僕们可以通过`auto.arima`、`ETS`、`GARCH`或者其它方式回测数据和弥补缺失值，不过那就比较费时了。根据以上的`imputeTS::na.seadec()`弥补来的数据的误差是`0`至`0.0001`，基本上还算可行。
