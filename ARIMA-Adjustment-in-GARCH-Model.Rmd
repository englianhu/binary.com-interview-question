...
## 读取程序包
pkg <- c('lubridate', 'plyr', 'dplyr', 'magrittr', 'stringr', 'rugarch', 'forecast', 'quantmod', 'microbenchmark')
suppressAll(lib(pkg))
rm(pkg)
```
...
## gather fx dataset.
#'@ fx <- llply(cr_code, function(x) suppressAll(getSymbols(x, from = (today('GMT') - 1) %m-% years(3), auto.assign = FALSE)))

#'@ if (!dir.exists('data/fx')) dir.create('data/fx')
#'@ saveRDS(fx, 'data/fx/fx3y.rds')
fx <- readRDS('data/fx/fx3y.rds') %>% na.omit
```
...
system.time(fit <- calc_fx(fx[[1]]))
system.time(fit2 <- calC(fx[[1]]))
```

以下僕运行三年数据测试后事先储存，然后直接读取。

```{r sim-pred1, warning = FALSE}
pred1 <- list()

for (i in seq(cr_code)) {
  pred1[[i]] <- ldply(price_type, function(y) {
    df = calC(fx[[names(cr_code)[i]]], currency = cr_code[i], price = y)
    df = data.frame(Date = index(df[[1]][1]), Type = paste0(names(df[[1]]), '.', y), df[[1]], df[[2]], t(df[[3]]))
    names(df)[4] %<>% str_replace_all('1', 'T+1')
    df
  })
  
  if (!dir.exists(paste0('data/fx', names(pred1[[i]])[3]))) dir.create(paste0('data/fx', names(pred1[[i]])[3]))
  saveRDS(pred1[[i]], paste0('data/fx', names(pred1[[i]])[3], '/pred1.', unique(pred1[[i]]$Date), '.rds'))
}; rm(i)

```

```{r sim-pred2, warning = FALSE}
pred2 <- list()

for (i in seq(cr_code)) {
  pred2[[i]] <- ldply(price_type, function(y) {
    df = calc_fx(fx[[names(cr_code)[i]]], currency = cr_code[i], price = y)
    df = data.frame(Date = index(df[[1]][1]), Type = paste0(names(df[[1]]), '.', y), df[[1]], df[[2]], t(df[[3]]))
    names(df)[4] %<>% str_replace_all('1', 'T+1')
    df
  })
  
  if (!dir.exists(paste0('data/fx', names(pred2[[i]])[3]))) dir.create(paste0('data/fx', names(pred2[[i]])[3]))
  saveRDS(pred2[[i]], paste0('data/fx', names(pred2[[i]])[3], '/pred2.', unique(pred2[[i]]$Date), '.rds'))
