loadLAD <- function(latestDate = NULL, dataSizeDay = 365) {
  ...
  if(file.exists('./shinyData/LAD.rds')) {
    if(readRDS('./shinyData/LAD.rds') %>% attributes %>% .$updated %>% as.Date < today()) {
      tryCatch({
        suppressAll(getSymbols('LAD', from = '2015-01-01'))
        }, error = function(e) stop('Kindly restart the shiny app.'))
      suppressAll(getSymbols('LAD', from = '2015-01-01'))
      saveRDS(LAD, file = './shinyData/LAD.rds')
  
      } else {
        LAD <- read_rds(path = './shinyData/LAD.rds')
        }
    } else {
      suppressAll(getSymbols('LAD', from = '2015-01-01'))
      saveRDS(LAD, file = './shinyData/LAD.rds')
