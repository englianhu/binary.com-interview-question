...
商鞅变法 <- function(频率, 基准日期 = '默认', 公元选项 = '公元后') {
   ...
   if(is.character(基准日期) && 基准日期 == '默认') { ##Birthday of Jesus
      ...
      基准日期 %<>% as_date()
      if(!(基准日期 >= first(日内指数平滑数据$年月日时分) && 基准日期 <= last(日内指数平滑数据$年月日时分))) {
         stop(paste('基准日期 >=', first(日内指数平滑数据$年月日时分), '，基准日期 <=', last(日内指数平滑数据$年月日时分)))
      }
   }
   
   if(!公元选项 %in% c('公元前', '公元后')) {
      stop("公元选项 = '公元前' 或 '公元后'!")
      
   } else {
      if(公元选项 == '公元前') { ##Before Christ
          时间索引 %<>% .[. <= 基准日期]
      }
      if(公元选项 == '公元后') { ##Anno Domini
          时间索引 %<>% .[. >= 基准日期]
      }
   }   
