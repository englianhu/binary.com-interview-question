...
task_progress <- function(mbase, timeID0 = NULL, scs = 60, .pattern = '^mts|^sets', .loops = TRUE) {
  ...
  if (.loops == TRUE) {
    while(1) {
      ...
      x = list.files(paste0('C:/Users/User/Documents/GitHub/binary.com-interview-question-data/fx/USDJPY/'), pattern = .pattern) %>% 
	      str_replace_all('.rds', '') %>% 
...
dsmp <- dsmp[,{
    ...
    sq_1m = 1:7200
    date_1m = as_date(index)
    .SD[,.(index = index, year = year, week = week, 
           wkdays = wkdays, sq_1m = sq_1m, sq = 1:.N, 
           date_1m = date_1m, close = close), ]}, ]
...
if(file.exists('C:/Users/User/Documents/GitHub/binary.com-interview-question-data/fx/USDJPY/dsmp.rds')) saveRDS(dsmp, 'data/fx/USDJPY/dsmp.rds')
```
...
```{r, eval=FALSE}
# --------- eval=FALSE ---------
timeID <- unique(dsmp$date_1m)
bse <- dsmp[year == 2016]$date_1m[1] #"2016-01-04" #1st trading date in 2nd year
timeID %<>% .[. > bse]
#timeID %<>% .[. > as_date('2016-01-04')]

if(is.data.table(dsmp)) dsmp %<>% lazy_dt
