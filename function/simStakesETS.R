simStakesETS <- function(mbase, .model = 'ZZZ', .damped = NULL, .additive.only = FALSE, 
                               .prCat = 'Op', .baseDate = ymd('2015-01-01'), .parallel = FALSE, 
                               .setPrice = 'Cl', .initialFundSize = 1000, .fundLeverageLog = FALSE, 
                               .filterBets = FALSE, .simulate = FALSE, .bootstrap = FALSE, 
                               .npaths = 5000) {
  ...
  #'@ source('./function/simETS.R', local = TRUE)
  ...
  fit1 <- simETS(mbase, .model = .model, .damped = .damped, .additive.only = .additive.only, 
                   .prCat = .prCat, .baseDate = .baseDate, .parallel = .parallel, 
                   .simulate = .simulate, .bootstrap = .bootstrap, .npaths = .npaths)
  fit1 <- data.frame(Date = index(fit1), coredata(fit1)) %>% tbl_df
  fit1 <- na.omit(fit1)
  
  ## forecast settlement price.
  fit2 <- simETS(mbase, .model = .model, .damped = .damped, .additive.only = .additive.only, 
                   .prCat = .setPrice, .baseDate = .baseDate, .parallel = .parallel, 
                   .simulate = .simulate, .bootstrap = .bootstrap, .npaths = .npaths)
