...
-With Various Error Distributions</span>]() applied few error distribuions onto dcc-garch model (dcc model will will modelled later):-
...
```
  ...
  We know from above table that model ``r gm.tbl %>% dplyr::filter(RR == max(RR, na.rm = TRUE)) %>% .$.id`` generates most return. Here I do combine the components/parameters of Garch models to get the best among them.
...
#'@ flsGarchM <- dir('./data', pattern = '^fGARCH.GJRGARCH+.+.hybrid')
flsGarchM <- dir('./data', pattern = '.hybrid')
...
```
  ...
  From above table, due to HiLo and LoHi price of ``r gm.tbl %>% dplyr::filter(RR == max(RR, na.rm = TRUE)) %>% .$.id`` model produced an error here I list below model which can generates highest ROI.
...
flsGarchM <- dir('./data', pattern = '.snorm.hybrid')
