...
## ========================= UI ========================================
ui <- fluidPage(
    h2('Testing cronR or Scheduled Task'), 
    textOutput('ctime', container = span), 
    #uiOutput('ctime'), 
    uiOutput('info'), 
    br(), 
    br(), 
...
server <- function(input, output, session) {
    ...
    output$ctime <- renderText({
        ...
        #reactiveTimer(1000, session)
        # format(Sys.time('GMT'))
        # as.POSIXct(strptime(now('GMT'), "%Y-%m-%d %H:%M:%S"))
        #format(now('GMT'))
        #as.character(as.POSIXct(now('GMT'), origin = '2018-06-10'))
        paste(as.character(now('GMT')), 'GMT just for display purpose.')
    })
    
    ## Variable to set the time when app stops automatically (HH:MM:SS)
    #timeStop <- '13:10:00'
    
    #calcTime <- as.POSIXct(timeStop, format = '%H:%M:%S' , tz = 'GMT')
    calcTime <- ymd_hms('2018-06-10 14:00:00', tz = 'GMT')
    if(now('GMT') > calcTime) {
        calcTime <- calcTime + minutes(1)
    }
    
    timecalcTime <- reactiveTimer(calcTime)
    #timecalcTime <- reactiveTimer(minutes(1))
    trick <- reactiveValues()
    trick$toFire <- FALSE
    
    observeEvent(timecalcTime(), {
        if (trick$toFire) {
            trn <- forecastData()
        } else {
            trick$toFire <- TRUE
    ...
    output$info <- renderUI({
        #invalidateLater(60000, session)
        reactiveTimer(60000, session)
        h2(paste("App will automatically start algorithmic calculation at ", calcTime))
    })
    }
