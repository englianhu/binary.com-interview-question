...
data_abn[c(1:3, (nrow(data_abn)-3):nrow(data_abn)),] %>% 
  kbl(caption = '1 min Close Price Dataset (abnor : Tuesday - Sunday)', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  ## https://public.tableau.com/en-us/gallery/100-color-palettes?gallery=votd
  row_spec(0, background = 'DimGrey', color = 'gold', bold = TRUE) %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
...
*source : `r paste0(dim(data_abn), collapse = ' x ')`*
...
## based on the report, here I retrived back and use the best model with different data size.
...
```{r, include = FALSE}
rm(bse, ctr, data_len, ets.m, hrz1, hrz2, i, lst_sq, timeID)
```
...
From this paper, we know that `r unique(sum$model)[1]` and `r unique(sum$model)[2]` is the <span style='color:strawberry'>βest model</span>.
...
As we can know from `ts` Here I use the <span style='color:strawberry'>βest model</span> but adjust the data size to compare.

```{r, warning = FALSE, message = FALSE, results='asis'}
MNN_MNZ <- list.files(paste0(.dtr, 'data/fx/USDJPY'), pattern = 'MNN|MNZ')
