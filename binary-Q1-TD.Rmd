...
pred.data$Date[length(pred.data$Date)] <- rmgarch::last(pred.data$LatestDate.GMT) + days(1)
...
pred %<>% group_by(Date) %>% 
    ...
    select(DateTime, Date, USDJPY.High, USDJPY.Low, USDJPY.Close, sq, Bid, Ask, Fct.High, Fct.Low, Fct.Close) %>% 
