...
*Table 2.2.1 : daily high-low price tick-data.*
...
Here I directly apply *GJR-GARCH*^[Kindly refer to [GJR-GARCH 模型](https://vlab.stern.nyu.edu/zh/doc/3?topic=mdls) for more information] model due to I had compared few statistical models and got the best fitted model. Kindly refer to [Binary.com Interview Q1](https://englianhu.github.io/2017/09/binary-forex-trading-Q1.html) or ([Alternate link](http://rpubs.com/englianhu/binary-forex-trading-Q1)) for the paper.
...
Here I wrote another extention page for Q1 which is analyse the multiple currencies and also models from minutes to daily. . You are feel free to browse over [Binary.com Interview Q1 (Extention)](http://rpubs.com/englianhu/binary-Q1E) or ([Alternate link](http://rpubs.com/englianhu/316133)). The paper compare and get the optimal predictive model based on the various number of observations.
...
Here I forecast 1 trading day advanced for daily Hi-Lo price.
...
Below I tidy the dataset.

```{r tidy-data1}
## Copied dataset.
mbase1 <- data.frame(Date = index(mbase), mbase) %>% tbl_df
pred.data1 <- pred.data %>% tbl_df

## Add `Date` column as forecasted Date.
pred.data1$Date <- lead(pred.data1$LatestDate.GMT)
pred.data1$Date[length(pred.data1$Date)] <- last(pred.data1$LatestDate.GMT) + days(1)

## Current daily price.
pred.data1A <- pred.data1 %>% 
    select(LatestDate.GMT, Lst.High, Lst.Low) %>% 
    mutate(Date = lead(LatestDate.GMT), 
           High = lead(Lst.High), Low = lead(Lst.Low)) %>% 
    select(Date, High, Low) %>% data.table %>% na.omit

pred.data1B <- pred.data1 %>% 
    select(Date, Fct.High, Fct.Low) %>% data.table

pred.data1 <- merge(pred.data1A, pred.data1B, by = 'Date')

## Merge dataset.
pred <- merge(tbl_df(mbase1), pred.data1, by = 'Date') %>% 
  tbl_df %>% select(-USDJPY.Volume, -USDJPY.Adjusted)
rm(mbase1, pred.data1, pred.data1A, pred.data1B)

## Here I test if the dataset scrapped from `quantmod::getSymbols(src = 'yahoo')` equal to FXCMTickData.
# pred %>% mutate(test.H = USDJPY.High == High, 
#                 test.L = USDJPY.Low == Low) %>% 
#     select(test.H, test.L) %>% unique
## A tibble: 1 x 2
#  test.H test.L
#  <lgl>  <lgl> 
#1 TRUE   TRUE

## Therefore here I removed the duplicated columns. 
pred %<>% select(-High, -Low)
...
data.frame(Category = c('High = ', 'Low = '), 
...
For the betting strategy, here I need to adjust a bit from **Binary.com Interview Q1** where fit the .
