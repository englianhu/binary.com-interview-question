...
## follow the seq to determine the buy or sell limit order stand within a day. Then the other side will be automatically switch to close transaction limit order but not placed another purchasing order.
pred <- ldply(split(pred, pred$Date), function(x) {
    x %<>% mutate(Trans = ifelse(!is.na(Bid), 'sell', 'buy'))
    x[nrow(x),]$Trans <- 'close'
    x
}) %>% tbl_df
...
```{r ROI-1}
sim_staking(pred) %>% 
  dplyr::select(Date, Edge1a, Edge1b, Edge2a, Edge2b, Buy, Sell, Trans, Profit, Bal) %>% 
  data.table
```
