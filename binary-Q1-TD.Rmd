...
```{r plot-data1A, echo = FALSE, results = 'asis'}
## Real price.
pl.RHigh <- pred %>% 
...
*Graph 3.2.1A* and *Graph 3.2.1B* compare the real price and forecast price. Following section will be compare the MSE (Mean Squared Error).
...
```{r data-summary4}
## Here I combine real-time dataset with the daily dataset.
pred <- merge(pred, HL_tick_data, by = 'Date') %>% 
  tbl_df %>% 
  arrange(DateTime)
...
#'@ f <- edge/odds
#'@ paste("edge =", edge ,"; f =",f)
##[1] "edge = 0.25 ; f = 0.0625"

## http://srdas.github.io/MLBook/Gambling.html#simulation-of-the-betting-strategy
kbase <- pred %>% mutate(
    p1 = 1/Fct.Low, Edge.H = p1 * Fct.High - (1 - p1), 
    p2 = 1/Fct.High, Edge.L = p2 * Fct.Low - (1 - p2), 
    f1 = Edge.H/Fct.Low, f2 = Edge.L/Fct.High) %>% 
    select(-USDJPY.Open, -USDJPY.High, -USDJPY.Low, -USDJPY.Close)

kbase %>% data.table
```

*Table 4.2.1 : Kelly ratio.*
>>>>>>> 1ae30d6a5a7d53e85a3ff48a520c05102a0014aa
