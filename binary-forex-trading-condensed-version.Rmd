...
garch.m <- llply(.variance.model, function(vm) {
  llply(.garchOrder, function(gO) {
    ft = ugarchspec(variance.model = list(model = vm, garchOrder = c(as.numeric(gO[1]), as.numeric(gO[2]))), distribution = 'std')
    if(vm == 'fGARCH'){
      llply(.sub.fGarch, function(sub.vm){
        ft = ugarchspec(variance.model = list(model = vm, garchOrder = c(as.numeric(gO[1]), as.numeric(gO[2]))), submodel = sub.vm, distribution = 'std')
      })
    }
  })
  })
