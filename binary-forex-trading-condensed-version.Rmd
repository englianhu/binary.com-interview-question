...
suppressAll(l_ply(c('last.R', 'has.Mn.R', 'simETS.R', 'simStakesETS.R', 'plotChart2.R', 'simGarch.R', 'simStakesGarch.R'), function(pkg) source(paste0('./function/', pkg))))
...
#### 2.1.6.2 Garch vs EWMA
  ...
  As I mentioned in first section which is the combination models will be more than 10,000, therefore I try to apply `acf()` and `pacf()` to determine the best fit value `p` and `q` for Garch model. You can refer to [Arima预测模型（R语言）](http://blog.csdn.net/desilting/article/details/39013825) and [时间序列分析---(ARIMA模型)](http://zhoulili1987619126.lofter.com/post/1cc8f7a3_74e7f10) for more information.
  
  - [Identifying the numbers of AR or MA terms in an ARIMA model](http://people.duke.edu/~rnau/411arim3.htm)
  - [8.7 ARIMA modelling in R](https://www.otexts.org/fpp/8/7)^[(a) The best model (with smallest AICc) is selected from the following four:] ^[ARIMA(2,d,2),] ^[ARIMA(0,d,0),] ^[ARIMA(1,d,0),] ^[ARIMA(0,d,1).]
  - [2.2 Partial Autocorrelation Function (PACF)](https://onlinecourses.science.psu.edu/stat510/node/62)
...
pacf(pq.op, lag.max = 100)
pacf(pq.hi, lag.max = 100)
pacf(pq.mn, lag.max = 100)
pacf(pq.lo, lag.max = 100)
pacf(pq.cl, lag.max = 100)

## determine the q value in Arima, Garch models.
acf(pq.op, lag.max = 100)
acf(pq.hi, lag.max = 100)
acf(pq.mn, lag.max = 100)
acf(pq.lo, lag.max = 100)
acf(pq.cl, lag.max = 100)
```
