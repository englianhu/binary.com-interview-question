...
fls <- dir('./data', pattern = 'fundAutoArima')
...
#### 2.1.6.2 Garch vs EWMA
  ...
  As I mentioned in first section which is the combination models will be more than 10,000, therefore I try to refer to `acf()` and `pacf()` to determine the best fit value `p` and `q` for Garch model. You can refer to below articles for more information.
...
#'@ .garchOrder.par <- expand.grid(0:2, 0:2, KEEP.OUT.ATTRS = FALSE) %>% mutate(PP = paste(Var1, Var2)) %>% .$PP %>% str_split(' ') %>% llply(as.numeric)
...
pacf(pq.op, lag.max = 100, main = 'PACF of Opening Price')
pacf(pq.hi, lag.max = 100, main = 'PACF of Highest Price')
pacf(pq.mn, lag.max = 100, main = 'PACF of Mean Price')
pacf(pq.lo, lag.max = 100, main = 'PACF of Lowest Price')
pacf(pq.cl, lag.max = 100, main = 'PACF of Closing Price')

## determine the q value in Arima, Garch models.
acf(pq.op, lag.max = 100, main = 'ACF of Opening Price')
acf(pq.hi, lag.max = 100, main = 'ACF of Highest Price')
acf(pq.mn, lag.max = 100, main = 'ACF of Mean Price')
acf(pq.lo, lag.max = 100, main = 'ACF of Lowest Price')
acf(pq.cl, lag.max = 100, main = 'ACF of Closing Price')
```

  Here I use a function to find the optimal value of `p` and `q` from (0,0) to (5,5) by refer to [<span style='color:blue'>R-ARMA(p,q)如何选找最小AIC的p,q值</span>](http://blog.csdn.net/u012543538/article/details/38663715).
...
pq.mn <- suppressWarnings(armaSearch(USDJPY.Mean, .method = 'ML')) #force to choose 'ML' since 'CSS-ML' error.
pq.lo <- suppressWarnings(armaSearch(Lo(mbase), .method = 'ML')) #force to choose 'ML' since 'CSS-ML' error.
pq.cl <- suppressWarnings(armaSearch(Cl(mbase)))

## From below comparison, we know that the 'CSS-ML' is better than 'ML'.
## 
#'@ > suppressWarnings(armaSearch(Cl(mbase), .method = 'ML'))
...
#'@ > suppressWarnings(armaSearch(Mn(USDJPY.Mean), .method = 'ML'))
...
#'@ > USDJPY.Mean = (Hi(mbase) + Lo(mbase)) / 2
#'@ > names(USDJPY.Mean) <- 'USDJPY.Mean'
#'@ > pq.mn <- suppressWarnings(armaSearch(USDJPY.Mean, .method = 'ML'))
# the min AIC = 1369.503 , p = 3 , q = 2 
#'@ > pq.lo <- suppressWarnings(armaSearch(Lo(mbase), .method = 'ML'))
# the min AIC = 1711.913 , p = 3 , q = 2 
#'@ > pq.cl <- suppressWarnings(armaSearch(Cl(mbase)))
# the min AIC = 1634.782 , p = 3 , q = 3
```

```{r dst, eval = FALSE, include = FALSE}
dstGarch <- llply(.dist.model.par, function(dst) {
  simStakesGarch(mbase, .solver = 'hybrid', .prCat = 'Mn', .prCat.method = 'ML', 
...
saveRDS(dstGarch, './data/dstGarch.rds')
```

```{r read-dst}
dstGarch <- readRDS('./data/dstGarch.rds')
...
```
  ...
  From above comparison of distribution used, we know that ``r dst.m %>% dplyr::filter(RR == max(RR)) %>% .$.id`` distribution generates most return. Now we know the best `p` and `q`, solver using `hybrid`, and best fitted distribution. Now we try to compare the Garch models.
...
garch.m <- suppressAll(llply(.variance.model.par, function(vm) {
  llply(pp, function(y) {
    llply(.garchOrder.par, function(gO) {
      llply(.solver.par[1], function(sv) { #only use 'hybrid' solver.
        llply(.dist.model.par, function(dst) {
          if(vm == 'fGARCH'){
            llply(.sub.fGarch.par, function(sub.vm) {
              z = simStakesGarch(mbase, .solver = sv, .prCat = y[1], 
                             .baseDate = ymd('2015-01-01'), 
                             .parallel = FALSE, .progress = 'text', 
                             .setPrice = y[2], .initialFundSize = 1000, 
                             .fundLeverageLog = FALSE, .filterBets = FALSE, 
                             .variance.model = list(model = vm, garchOrder = c(gO[1], gO[2]), submodel = sub.vm, external.regressors = NULL, variance.targeting = FALSE), 
                             ...
                             .dist.model = dst, start.pars = list(), fixed.pars = list())
              
              txt1 <- paste0('saveRDS(z', ', file = \'./data/', vm, '.', y, '.', gO[1], '.', gO[2], '.', dst, '.', sv, '.', sub.vm, '.', y[1], y[2], '.rds\')')
        eval(parse(text = txt1))
        cat(paste0(txt1, ' done!', '\n'))
        rm(z)
              ...
              z = simStakesGarch(mbase, .solver = sv, .prCat = y[1], 
                             ...
                             .setPrice = y[2], .initialFundSize = 1000, 
                             .fundLeverageLog = FALSE, .filterBets = FALSE, 
                             .variance.model = list(model = vm, garchOrder = c(gO[1], gO[2]), submodel = NULL, external.regressors = NULL, variance.targeting = FALSE), 
                             .mean.model = list(armaOrder = c(1, 1), include.mean = TRUE, archm = FALSE, archpow = 1, arfima = FALSE, external.regressors = NULL, archex = FALSE), 
                             .dist.model = dst, start.pars = list(), fixed.pars = list())
              
              txt1 <- paste0('saveRDS(z', ', file = \'./data/', vm, '.', y, '.', gO[1], '.', gO[2], '.', dst, '.', sv, '.', y[1], y[2], '.rds\')')
        eval(parse(text = txt1))
        cat(paste0(txt1, ' done!', '\n'))
        rm(z)
...
```{r latest}
garch.m <- suppressAll(llply(.variance.model.par, function(vm) {
    llply(pp[19], function(y) {
            llply(.solver.par[1], function(sv) { #only use 'hybrid' solver.
                llply(.dist.model.par[2], function(dst) { #snorm is the best from comparison.
                    if(vm == 'fGARCH'){
                        llply(.sub.fGarch.par, function(sub.vm) {
                            z = simStakesGarch(mbase, .solver = sv, .prCat = y[1], 
                                               .prCat.method = 'ML', 
                                               ...
                                               .parallel = FALSE, .progress = 'text', 
                                               .setPrice = y[2], 
                                               .setPrice.method = 'CSS-ML', 
                                               .initialFundSize = 1000, 
                                               .fundLeverageLog = FALSE, .filterBets = FALSE, 
                                               .variance.model = list(model = vm, garchOrder = c(1, 1), submodel = sub.vm, external.regressors = NULL, variance.targeting = FALSE), 
                                               .mean.model = list(armaOrder = c(1, 1), include.mean = TRUE, archm = FALSE, archpow = 1, arfima = FALSE, external.regressors = NULL, archex = FALSE), 
                                               .dist.model = dst, start.pars = list(), fixed.pars = list())
                            
                            txt1 <- paste0('saveRDS(z', ', file = \'./data/', vm, '.', y, '.', dst, '.', sv, '.', sub.vm, '.', y[1], y[2], '.rds\')')
                            eval(parse(text = txt1))
                            cat(paste0(txt1, ' done!', '\n'))
                            rm(z)
                        ...
                        z = simStakesGarch(mbase, .solver = sv, .prCat = y[1], 
                                           ...
                                           .initialFundSize = 1000, 
                                           .fundLeverageLog = FALSE, .filterBets = FALSE, 
                                           .variance.model = list(model = vm, garchOrder = c(1, 1), submodel = NULL, external.regressors = NULL, variance.targeting = FALSE), 
                                           .mean.model = list(armaOrder = c(1, 1), include.mean = TRUE, archm = FALSE, archpow = 1, arfima = FALSE, external.regressors = NULL, archex = FALSE), 
                                           .dist.model = dst, start.pars = list(), fixed.pars = list())
                        
                        txt1 <- paste0('saveRDS(z', ', file = \'./data/', vm, '.', y, '.', dst, '.', sv, '.', y[1], y[2], '.rds\')')
                        eval(parse(text = txt1))
                        cat(paste0(txt1, ' done!', '\n'))
                        rm(z)
...
```
garch.m <- llply(.variance.model.par, function(vm) {
    llply(pp, function(y) {
        llply(.garchOrder.par, function(gO) {
            llply(.solver.par[1], function(sv) { #only use 'hybrid' solver.
                llply(.dist.model.par, function(dst) {
                    if(vm == 'fGARCH'){
                        llply(.sub.fGarch.par, function(sub.vm) {
                            txt2 = paste0("z = simStakesGarch(mbase, .solver = sv, .prCat = y[1], .baseDate = ymd('2015-01-01'), .parallel = FALSE, .progress = 'text', .setPrice = y[2], .initialFundSize = 1000, .fundLeverageLog = FALSE, .filterBets = FALSE, .variance.model = list(model = vm, garchOrder = c(gO[1], gO[2]), submodel = sub.vm, external.regressors = NULL, variance.targeting = FALSE), .mean.model = list(armaOrder = c(1, 1), include.mean = TRUE, archm = FALSE, archpow = 1, arfima = FALSE, external.regressors = NULL, archex = FALSE), .dist.model = dst, start.pars = list(), fixed.pars = list()); saveRDS(z, file = paste0('./data/', vm, '.', dst, '.', sv, '.', sub.vm, '.', y[1], y[2], '.rds')); rm(z)")
                            eval(parse(text = txt2))
                            cat(paste0(txt2, ' done!', '\n'))
                            
                        }, .progress = 'text')
                    } else {
                        txt2 = paste0("z = simStakesGarch(mbase, .solver = sv, .prCat = y[1], .baseDate = ymd('2015-01-01'), .parallel = FALSE, .progress = 'text', .setPrice = y[2], .initialFundSize = 1000, .fundLeverageLog = FALSE, .filterBets = FALSE, .variance.model = list(model = vm, garchOrder = c(gO[1], gO[2]), submodel = NULL, external.regressors = NULL, variance.targeting = FALSE), .mean.model = list(armaOrder = c(1, 1), include.mean = TRUE, archm = FALSE, archpow = 1, arfima = FALSE, external.regressors = NULL, archex = FALSE), .dist.model = dst, start.pars = list(), fixed.pars = list()); saveRDS(z, file = paste0('./data/', vm, '.', dst, '.', sv, '.', y[1], y[2], '.rds')); rm(z)")
                            eval(parse(text = txt2))
                            cat(paste0(txt2, ' done!', '\n'))
