...
# Verify Expectation Maximization
...
miss.timeID <- missing %>% .['path'] %>% 
  apply(1, function(x) {
    x %>% str_extract_all('[0-9]{4}-[0-9]{2}-[0-9]{2}') %>% unlist
  }) %>% as.Date %>% sort %>% unique
```
...
united.dateID <- ldply(mdate, dplyr::count, date) %>% tbl_df %>% 
  dplyr::filter(n == 7) %>% arrange(date) %>% 
  spread(.id, n) %>% 
...
```{r filter-data2, eval=FALSE, echo=FALSE}
## filter gmds
gmds2 <- c('sGARCH', 'fGARCH.GARCH', 'fGARCH.TGARCH', 'fGARCH.NGARCH', 'fGARCH.NAGARCH', 'fGARCH.GJRGARCH', 'gjrGARCH', 'iGARCH', 'csGARCH')
...
Here I try to review on paper **Binary.com Interview Q1** to compare the accuracy of prediction.
