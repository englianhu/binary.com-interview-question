...
conflicted::conflicts_prefer(tidyft::slice, .quiet = TRUE)
...
ddsmp <- na.omit(dsmp) |> 
    {\(df) df |> 
         dplyr::mutate(
             sq = 1:n(), 
             date = as_date(index), 
             quarter = lubridate::quarter(index), 
...
fit1A <- train1A |> 
    model(
        ets = ETS(close ~ error('M') + trend('Ad') + season('A')), 
        ets_bc = ETS(box_cox(close, 0.3) ~ error('A') + trend('Ad') + season('A')),
        arima = ARIMA(close), 
        rw = RW(close), 
...
01. [High-Performance Time Series Forecasting in R & Python](https://www.youtube.com/watch?v=elQb4VzRINg)
02. [Forecasting: Principles and Practice (3rd ed)](https://otexts.com/fpp3)
03. [A Guide to Time Series Forecasting in R You Should Know](https://www.simplilearn.com/tutorials/data-science-tutorial/time-series-forecasting-in-r)
