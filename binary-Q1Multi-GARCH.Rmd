...
timeID <- HiLo.base %>% index %>% .[. >= timeID0] %>% c(., last(.) + days(1))
tmID <- list.files('data/fx/USDJPY', pattern = '^DCC') %>% 
  str_extract_all('[0-9]{4}-[0-9]{2}-[0-9]{2}') %>% unlist %>% ymd
timeID %<>% .[!. %in% tmID]
...
## only use USDJPY trading day.
timeID <- HiLo.base %>% index %>% .[. >= timeID0] %>% c(., last(.) + days(1))
tmID <- list.files('data/fx/USDJPY', pattern = '^aDCC') %>% 
  str_extract_all('[0-9]{4}-[0-9]{2}-[0-9]{2}') %>% unlist %>% ymd
timeID %<>% .[!. %in% tmID]
timeID %<>% .[. > ymd('2013-01-01')]
...
## only use USDJPY trading day.
timeID <- HiLo.base %>% index %>% .[. >= timeID0] %>% c(., last(.) + days(1))
tmID <- list.files('data/fx/USDJPY', pattern = '^FDCC') %>% 
  str_extract_all('[0-9]{4}-[0-9]{2}-[0-9]{2}') %>% unlist %>% ymd
timeID %<>% .[!. %in% tmID]
timeID %<>% .[. > ymd('2013-01-01')]
...
```{r DCC-compare}
fls <- list.files('data/fx/USDJPY', pattern = 'DCC')

## get only MSE and AIC/BIC but ommit VaR.
MSE <- ldply(fls, function(x) {
