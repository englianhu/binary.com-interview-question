...
```{r DCC-HLVARF, warning=FALSE, eval=FALSE, echo=FALSE}
## ------------- Simulate mv_fx() ----------------------
...
for (dt in timeID) {
  ...
  for (i in seq(cr_code[7])) {
    ...
    if (dt %in% timeID2) {
      ...
      if (!dir.exists(paste0('data/fx/', substr(names(DCC.GARCH.USDJPY[[i]])[3], 1, 6)))) 
        dir.create(paste0('data/fx/', substr(names(DCC.GARCH.USDJPY[[i]])[3], 1, 6)))
      
      saveRDS(DCC.GARCH.USDJPY[[i]], paste0(
        'data/fx/', substr(names(DCC.GARCH.USDJPY[[i]])[3], 1, 6), '/DCC.GARCH.USDJPY.', 
