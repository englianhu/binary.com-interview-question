  ...
  for (i in seq(cr_code[7])) {
    ...
    if (dt %in% timeID2) {
      ...
      saveRDS(mvgoGARCH.USDJPY[[i]], paste0(
        'data/fx/', subdir, '/mvgoGARCH.USDJPY.HL.', md[1], '.', 
        .dist.models[1], '.', 
...
for (dt in timeID) {
  ...
  for (i in seq(cr_code[7])) {
    ...
    if (dt %in% timeID2) {
      ...
      saveRDS(mvgoGARCH.USDJPY[[i]], paste0(
        'data/fx/', subdir, '/mvgoGARCH.USDJPY.HL.', md[2], '.', 
        .dist.models[1], '.', 
        unique(mvgoGARCH.USDJPY[[i]]$Date), '.rds'))
...
for (dt in timeID) {
  ...
  for (i in seq(cr_code[7])) {
    ...
    if (dt %in% timeID2) {
      ...
      saveRDS(mvgoGARCH.USDJPY[[i]], paste0(
        'data/fx/', subdir, '/mvgoGARCH.USDJPY.HL.', md[3], '.', 
        .dist.models[1], '.', 
        unique(mvgoGARCH.USDJPY[[i]]$Date), '.rds'))
...
flv <- llply(td, grep, fls, value = TRUE) %>% unlist
...
Similar with previous table, the observation number `n` for `Uni-gjrGARCH` should be devided by `2`.
