...
以上图表显示数据`年月日时分`，由`r paste(range(样本二零一八半年$年月日时分), collapse = ' 至 ')`，接下来的科研一律使用同样的半年数据，是为了在回测多元统计模型，才能获知并筛选最优统计模型。
...
在此使用经过过滤的`样本二零一八半年`数据，再次运算以上函数，再与其它统计模型比较预测值误差，以证实上篇**（第III部）**的结论是否相符。
