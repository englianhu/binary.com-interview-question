...
for (i in 1:8) { #Here I print 8 elements as examples
...
timeID %<>% .[. >= bse]
#timeID %<>% .[. >= as_date('2016-01-04')]
data_len <- 22 * 1440 #last 31680 observations DT[(.N - (data_len - 1)):.N]
hrz1 <- 1440
...
for (i in 1:length(timeID)) {
  ...
  if(i == 1) {
    ...
    saveRDS(sets, paste0(
      dtr, 'data/fx/USDJPY/ts_31680_1440.', 
      as_date(sets$index[1]), '.rds'))

    cat('\n\n')
    cat(i, '=', paste0('~/data/fx/USDJPY/ts_31680_1440.', 
                       as_date(sets$index[1]), '.rds saved!\n\n'))
    cat('\n\n')
...
timeID %<>% .[. >= bse]
#timeID %<>% .[. >= as_date('2016-01-04')]
data_len <- 52 * 5 * 1440 #last 374400 observations DT[(.N - (data_len - 1)):.N]
hrz1 <- 1440
