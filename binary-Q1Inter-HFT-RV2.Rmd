...
```{r long-fmt, warning = FALSE, message = FALSE, results = 'asis'}
if(!exists('mds_ets_MNN_intraday')) {
  mds_ets_MNN_intraday <- read_rds(
    paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday.rds')) }
...
```{r long-fLF, warning = FALSE, message = FALSE, results = 'asis'}
...
```{r long-fLT, warning = FALSE, message = FALSE, results = 'asis'}
...
```{r plt-fLF, warning = FALSE, message = FALSE, results = 'asis'}
...
```{r plt-fLT, warning = FALSE, message = FALSE, results = 'asis'}
## https://www.r-graph-gallery.com/line-chart-several-groups-ggplot2.html
...
```{r rp-fLF, warning = FALSE, message = FALSE, results = 'asis'}
...
```{r rp-fLT, warning = FALSE, message = FALSE, results = 'asis'}
...
```{r sum-rp, warning = FALSE, message = FALSE, results = 'asis'}
rm(list = ls())
...
if(!exists('mds_ets_MNN_intraday_1min')) {
  mds_ets_MNN_intraday_1min <- read_rds(
    paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min.rds')) }

mds_ets_MNN_intraday_1min_cmp <- mds_ets_MNN_intraday_1min %>% 
  #tidyr::unite(model, t_series:part) %>% 
  tidyr::unite(model, t_series:fc_min) %>% 
  as.data.table %>% 
  .[, .(index, model, mk.price, fc.price)] %>% as.data.table

mds_ets_MNN_intraday_1min_cmp <- as.data.table(mds_ets_MNN_intraday_1min_cmp)

## ----------------------------------------
## fromLast = FALSE
mds_ets_MNN_intraday_1min_cmp_fLF <- unique(
  mds_ets_MNN_intraday_1min_cmp[, .(index, model, mk.price, fc.price)], fromLast = FALSE)

## save dataset in data.table format
saveRDS(mds_ets_MNN_intraday_1min_cmp_fLF, 
        paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_cmp_fLF.rds'))

## ----------------------------------------
## fromLast = TRUE
mds_ets_MNN_intraday_1min_cmp_fLT <- unique(
  mds_ets_MNN_intraday_1min_cmp[, .(index, model, mk.price, fc.price)], fromLast = TRUE)

## save dataset in data.table format
saveRDS(mds_ets_MNN_intraday_1min_cmp_fLT, 
        paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_cmp_fLT.rds'))

## ----------------------------------------
## convert to long format for plot chart
## fromLast = FALSE
prc <- unique(mds_ets_MNN_intraday_1min_cmp[, .(index, model, mk.price)], fromLast = FALSE)
prc <- prc[, model := 'actual'][]
setnames(prc, old = 'mk.price', new = 'price')
prc <- unique(prc, fromLast = FALSE)

mds_ets_MNN_intraday_1min_grph <- unique(
  mds_ets_MNN_intraday_1min_cmp[, .(index, model, fc.price)], fromLast = FALSE)
setnames(mds_ets_MNN_intraday_1min_grph, old = 'fc.price', new = 'price')

mds_ets_MNN_intraday_1min_grph <- rbind(mds_ets_MNN_intraday_1min_grph, prc)
mds_ets_MNN_intraday_1min_grph_fLF <- data.table(mds_ets_MNN_intraday_1min_grph)[order(index)]
rm(prc)

## save dataset in data.table format
saveRDS(mds_ets_MNN_intraday_1min_grph_fLF, 
        paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_grph_fLF.rds'))

## ----------------------------------------
## fromLast = TRUE
prc <- unique(mds_ets_MNN_intraday_1min_cmp[, .(index, model, mk.price)], fromLast = TRUE)
prc <- prc[, model := 'actual'][]
setnames(prc, old = 'mk.price', new = 'price')
prc <- unique(prc, fromLast = TRUE)

mds_ets_MNN_intraday_1min_grph <- unique(
  mds_ets_MNN_intraday_1min_cmp[, .(index, model, fc.price)], fromLast = TRUE)
setnames(mds_ets_MNN_intraday_1min_grph, old = 'fc.price', new = 'price')

mds_ets_MNN_intraday_1min_grph <- rbind(mds_ets_MNN_intraday_1min_grph, prc)
mds_ets_MNN_intraday_1min_grph_fLT <- data.table(mds_ets_MNN_intraday_1min_grph)[order(index)]
rm(prc)

## save dataset in data.table format
saveRDS(mds_ets_MNN_intraday_1min_grph_fLT, 
        paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_grph_fLT.rds'))
```
...
## plot sample data
mds_ets_MNN_intraday_1min[c(1:3, (nrow(mds_ets_MNN_intraday_1min)-3):nrow(mds_ets_MNN_intraday_1min)),] %>% 
  kbl(caption = 'Data Sample (ETS MNN intraday forecast 1min)', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  ## https://public.tableau.com/en-us/gallery/100-color-palettes?gallery=votd
  row_spec(0, background = 'DimGrey', color = 'gold', bold = TRUE) %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
...
*source : `r paste0(dim(mds_ets_MNN_intraday_1min), collapse = ' x ')`*

There has a small mistake in previous paper [<span style='color:#DE5D83; background-color:black;'>*Deriv.com*</span> - *Interday High Frequency Trading Models Comparison* <span style='color:RoyalBlue'>*Review (Part I)*</span>](https://rpubs.com/englianhu/binary-Q1Inter-HFT-RV1) but doesn't affect since I only want to compare the models and here I use the same `MNN` model and filter `unique(fromLast = FALSE)` and `unique(fromLast = TRUE)`.

```{r chk-fls}
if(!exists('mds_ets_MNN_intraday_1min_grph_fLF')) {
  mds_ets_MNN_intraday_1min_grph_fLF <- readRDS(
    paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_grph_fLF.rds')) }

if(!exists('mds_ets_MNN_intraday_1min_grph_fLT')) {
  mds_ets_MNN_intraday_1min_grph_fLT <- readRDS(
    paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_grph_fLT.rds')) }
```

```{r long-fLF, warning = FALSE, message = FALSE, results = 'asis'}
data.frame(mds_ets_MNN_intraday_1min_grph_fLF)[
  c(1:5, (nrow(mds_ets_MNN_intraday_1min_grph_fLF)-5):nrow(mds_ets_MNN_intraday_1min_grph_fLF)),] %>% 
  kbl(caption = 'Data Sample', escape = FALSE) %>% 
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
  column_spec(2, background = 'grey') %>% 
...
*source : `r paste0(dim(mds_ets_MNN_intraday_1min_grph_fLF), collapse = ' x ')`*

```{r long-fLT, warning = FALSE, message = FALSE, results = 'asis'}
data.frame(mds_ets_MNN_intraday_1min_grph_fLT)[
  c(1:6, (nrow(mds_ets_MNN_intraday_1min_grph_fLT)-5):nrow(mds_ets_MNN_intraday_1min_grph_fLT)),] %>% 
  kbl(caption = 'Data Sample', escape = FALSE) %>% 
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  column_spec(1, background = 'CornflowerBlue') %>% 
  column_spec(2, background = 'grey') %>% 
...
*source : `r paste0(dim(mds_ets_MNN_intraday_1min_grph_fLT), collapse = ' x ')`*
...
```{r plt-fLF, warning = FALSE, message = FALSE, results = 'asis'}
## https://www.r-graph-gallery.com/line-chart-several-groups-ggplot2.html
plt <- mds_ets_MNN_intraday_1min_grph_fLF %>% 
  ggplot(aes(x = index, y = price, group = model, color = model)) + 
  geom_line() + 
...
```{r plt-fLT, warning = FALSE, message = FALSE, results = 'asis'}
## https://www.r-graph-gallery.com/line-chart-several-groups-ggplot2.html
plt <- mds_ets_MNN_intraday_1min_grph_fLT %>% 
  ggplot(aes(x = index, y = price, group = model, color = model)) + 
  geom_line() + 
...
## report for mds_ets_MNN_intraday_cmp_fLF
if(!exists('mds_ets_MNN_intraday_1min_cmp_fLF')) {
    mds_ets_MNN_intraday_1min_cmp_fLF <- read_rds(
        paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_cmp_fLF.rds')) }

if(!is.data.table('mds_ets_MNN_intraday_1min_cmp_fLF')) {
  mds_ets_MNN_intraday_1min_cmp_fLF %<>% as.data.table }

mds_ets_MNN_intraday_1min_cmp_fLF <- unique(
  mds_ets_MNN_intraday_1min_cmp_fLF, fromLast = FALSE, by = c('index', 'model')) %>% 
  na.omit

rp_1min_fLF <- mds_ets_MNN_intraday_1min_cmp_fLF[, {
  mk.price = mk.price
  fc.price = fc.price
  .SD[, .(.N, 
          mae = MLmetrics::MAE(y_true = mk.price, y_pred = fc.price), 
...
## report
saveRDS(rp_1min_fLF, paste0(.dtr, 'data/fx/USDJPY/rp_1min_fLF.rds'))
```

```{r rp-fLF, warning = FALSE, message = FALSE, results = 'asis'}
rp_1min_fLF <- readRDS(paste0(.dtr, 'data/fx/USDJPY/rp_1min_fLF.rds'))

rp_1min_tbl <- rp_1min_fLF %>% 
  dplyr::mutate(
    mae = ifelse(
      rank(mae) <= 3, 
      cell_spec(
    ...
    mse = ifelse(
      ...
      cell_spec(
        paste0(round(mse, 7), ' (rank: ', sprintf('%1.f', rank(mse)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
  kbl(caption = 'Accurcy Report (Intraday Forecasted 1 min)', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  #column_spec(1, background = 'CornflowerBlue') %>% 
  column_spec(1, background = '#556DAC') %>% 
...
*source : `r paste0(dim(rp_1min_tbl), collapse = ' x ')`*

```{r include = FALSE}
if(exists('rp_1min_tbl')) rm(rp_1min_tbl)
```
...
## report for mds_ets_MNN_intraday_1min_cmp_fLT
if(!exists('mds_ets_MNN_intraday_1min_cmp_fLT')) {
    mds_ets_MNN_intraday_1min_cmp_fLT <- read_rds(
        paste0(.dtr, 'data/fx/USDJPY/mds_ets_MNN_intraday_1min_cmp_fLT.rds')) }

if(!is.data.table('mds_ets_MNN_intraday_1min_cmp_fLT')) {
  mds_ets_MNN_intraday_1min_cmp_fLT %<>% as.data.table }

mds_ets_MNN_intraday_1min_cmp_fLT <- unique(
  mds_ets_MNN_intraday_1min_cmp_fLT, fromLast = TRUE, by = c('index', 'model')) %>% 
  na.omit

rp_1min_fLT <- mds_ets_MNN_intraday_1min_cmp_fLT[, {
  mk.price = mk.price
  fc.price = fc.price
  .SD[, .(.N, 
          mae = MLmetrics::MAE(y_true = mk.price, y_pred = fc.price), 
...
## report
saveRDS(rp_1min_fLT, paste0(.dtr, 'data/fx/USDJPY/rp_1min_fLT.rds'))
```

```{r rp-fLT, warning = FALSE, message = FALSE, results = 'asis'}
rp_1min_fLT <- readRDS(paste0(.dtr, 'data/fx/USDJPY/rp_1min_fLT.rds'))

rp_1min_tbl <- rp_1min_fLT %>% 
  dplyr::mutate(
    mae = ifelse(
      rank(mae) <= 3, 
      cell_spec(
    ...
    mse = ifelse(
      ...
      cell_spec(
        paste0(round(mse, 7), ' (rank: ', sprintf('%1.f', rank(mse)), ')'), 
        color = 'grey', italic = TRUE))) %>% 
  kbl(caption = 'Accurcy Report (Intraday Forecasted 1 mins)', escape = FALSE) %>% 
  ## https://www.w3schools.com/cssref/css_colors.asp
  row_spec(0, background = 'DimGrey', color = 'yellow') %>% 
  #column_spec(1, background = 'CornflowerBlue') %>% 
  column_spec(1, background = '#556DAC') %>% 
...
## report for rp_1min_fLF and 
if(!exists('rp_1min_fLF')) {
    rp_1min_fLF <- read_rds(
        paste0(.dtr, 'data/fx/USDJPY/rp_1min_fLF.rds')) %>% 
      as.data.table }

## report for rp_1min_fLT and 
if(!exists('rp_1min_fLT')) {
    rp_1min_fLT <- read_rds(
        paste0(.dtr, 'data/fx/USDJPY/rp_1min_fLT.rds')) %>% 
      as.data.table }

rp_1min_fLF <- rp_1min_fLF[, arrange := 'from_first'][]
rp_1min_fLT <- rp_1min_fLT[, arrange := 'from_last'][]

rp_1min <- rbindlist(list(rp_1min_fLF, rp_1min_fLT))
rp_1min <- rp_1min[, .(model, arrange, N, mae, mape, rmse, smape, mse)]
rp_1min %<>% unique

#sum <- rp_1min[mae == min(mae) | mape == min(mape) | rmse == min(rmse) | smape == min(smape) | mse == min(mse)]
...
*source : `r paste0(dim(rp_1min), collapse = ' x ')`*
