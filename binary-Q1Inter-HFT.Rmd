...
Above table shows the frequency of the dataset is not tally. Kindly refer to section [Seasonal Data] or [Seasonal periods](https://robjhyndman.com/hyndsight/seasonal-periods/) to know the seasonality dataset.
...
## function
as.POSIXct.no.dst <- function (x, tz = '', 
                               format='%Y-%m-%d %H:%M', offset='+0100', ...) {
  x <- paste(x, offset)
...
```{r}
## convert timezone, however the daylight-saving need to consider.
Y2015W1 %<>% mutate(
...
Y2015W1 %>% 
  dplyr::select(index, close) %>% 
  mutate(index = as.Date(index, tz='Europe/Athens')) %>% 
  ddply(.(index), summarise, n = length(index)) %>% 
  tbl_df
```
...
Now I convert all dataset again from `UTC` to `UTC+2` to be a constant weekly seasonal dataset. Due to the trading day is 5 days and 2 rest days, therefore I set a weekly seasonal period instead of daily.
