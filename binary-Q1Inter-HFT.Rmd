...
## 
if(is.double(fr_ts$forecast$index[1])){
    sq <- smp %>% 
        tail(1) %>% 
        index
    if(weekdays(sq) == '土曜日'|weekdays(sq) == 'Saturday') sq <- sq + days(2)
    sq <- seq(from = sq + minutes(1), sq + days(1), by = 'min')
    fr_ts$forecast$index <- sq
    
} else {
    fr_ts$forecast$index <- data_m1$index[
        (which(data_m1$index == smp %>% 
                   index %>% 
                   xts::last()) + 1):(
                       which(data_m1$index == smp %>% 
...
saveRDS(fr_ts, paste0('data/fx/USDJPY/fr_ts.sample.wk.1440.', 
                         as_date(fr_ts$forecast$index[1]), '.rds'))

cat(paste0(
    'data/fx/USDJPY/fr_ts.sample.wk.1440.', 
    as_date(fr_ts$forecast$index[1]), '.rds saved!\n'))
```
...
for (dt in timeID) {
  ...
  if(is.double(sarimats$forecast$index[1])){
    sq <- smp %>% 
      tail(1) %>% 
      index
    if(weekdays(sq) == '土曜日'|weekdays(sq) == 'Saturday') sq <- sq + days(2)
    sq <- seq(from = sq + minutes(1), sq + days(1), by = 'min')
    sarimats$forecast$index <- sq
    
  } else {
    sarimats$forecast$index <- data_m1$index[
    (which(data_m1$index == smp %>% 
             index %>% 
             xts::last()) + 1):(
     which(data_m1$index == smp %>% 
  ...
  saveRDS(sarimats, paste0('data/fx/USDJPY/sarimats.wk.1440.', 
                       as_date(sarimats$forecast$index[1]), '.rds'))
  
  cat(paste0(
    'data/fx/USDJPY/sarimats.wk.1440.', 
    as_date(sarimats$forecast$index[1]), '.rds saved!\n'))
  }
```
...
##
if(is.numeric(index(fr_msts)[1])){
  sq <- smp %>% 
    tail(1) %>% 
    index
  if(weekdays(sq) == '土曜日'|weekdays(sq) == 'Saturday') sq <- sq + days(2)
  sq <- seq(from = sq + minutes(1), sq + days(1), by = 'min')
  n <- nrow(fr_msts) / length(sq)

  fr_msts <- cbind(index = rep(sq, n), fr_msts) %>% 
    as_tibble

  } else {
    stop('index of dataset does not assign.')
    #fr_msts$forecast$index <- data_m1$index[
    #(which(data_m1$index == smp %>% 
    #         index %>% 
    #         xts::last()) + 1):(
    # which(data_m1$index == smp %>% 
  ...
  saveRDS(fr_msts, paste0('data/fx/USDJPY/fr_msts.sample.dy.wk.', 
                       as_date(fr_msts$index[1]), '.rds'))
  
  cat(paste0(
    'data/fx/USDJPY/fr_msts.sample.dy.wk.', 
    as_date(fr_msts$index[1]), '.rds saved!\n'))
```
...
for (dt in timeID) {
  ...
  if(is.numeric(index(sarimamsts)[1])){
    sq <- smp %>% 
      tail(1) %>% 
      index
    if(weekdays(sq) == '土曜日'|weekdays(sq) == 'Saturday') sq <- sq + days(2)
...
Here I summarise 1440 of 1 min data to be 1 day and choose the best fitted model. Below table compares the models.
...
sarimats %>% as_tibble %>% bind_cols(index = index(smp), .)
sarimamsts %>% as_tibble %>% bind_cols(index = index(smp), .)
```
