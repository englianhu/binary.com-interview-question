...
*High Frequency Financial Time Series Prediction - Machine Learning Approach* introduce multilayer modelling for high-frequency-trading. *binary.com Interview Question I - Tick-Data-HiLo For Daily Trading <span style='color:red'>(Blooper)</span>* tried to use Hi-Lo daily dataset for modelling but failed. The paper recommend to use complete itraday dataset.
...
Kindly refer to section [Seasonal Data] or [Seasonal periods](https://robjhyndman.com/hyndsight/seasonal-periods/) to know the seasonality dataset. *High Frequency GARCH: The multiplicative component GARCH (mcsGARCH) model* use the dataset start from `00:01:00` but not `00:00:00`, therefore above dataset shows the last observation will be the start of next day.
...
tsz <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/tsz.rds') %>% 
...
Y2015W1_min1 %>% tk_ts(end = c(3, 7200), frequency = 1440) %>% .[,1:ncol(.)] %>% head #1440 * 5 = 7200

Y2015W1_min1 %>% head %>% zooreg(frequency = 1440)
...
Y2015W1_min1 %>% head %>% msts(seasonal.periods=c(1440, 7200))
```
...
fs1 <- list.files('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY', pattern = '^Y[0-9]{4}W[0-9]{1,2}_m1.rds$') %>% str_replace_all('.rds', '')

## Read raw dataset.
#'@ eval(parse(text = paste0(fs, "<- readRDS('", fls[[7]], "')")))
```

```{r warning=FALSE}
## Read raw dataset.
eval(parse(text = paste0(fs1[1], "<- readRDS('", fls1[[5]][1], "') %>% tbl_df")))
...
#    y <- readRDS(x) %>% dplyr::filter(DateTime == head(DateTime, 1)|
...
#        dplyr::select(-nch)
#    }) %>% bind_rows %>% tbl_df %>% arrange(DateTime)
# saveRDS(tsz2, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/tsz2.rds')

## The daylight saving convertion in not tally.
tsz2 <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/tsz2.rds') %>% dplyr::rename(index = DateTime) %>% 
...
[binary.com 面试试题 I - 单变量数据缺失值管理](http://rpubs.com/englianhu/handle-missing-value) and [binary.com面试试题 I - 单变量数据缺失值管理 II](http://rpubs.com/englianhu/428318) compares the missing values dataset and refill the missing value, the paper concludes that the repaired dataset is workable since the MSE is very low. The paper compares tick-data-to-1min-data and also 1min-data where both datasets gather from FXCM.
...
## minute 1 dataset
#tsz2 <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/tsz2.rds')
tsz2 %>% dplyr::count(diff) %>% 
  kable(caption = 'Count data point') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>%
  scroll_box(width = '100%', height = '400px')
...
data_m1 <- llply(fls1[[5]], function(x) {
    y <- readRDS(x) %>% dplyr::rename(index = DateTime) %>% 
...
data_tm1 <- llply(fls[[7]], function(x) {
    y <- readRDS(x) %>% convertOHLC(combine = TRUE)
...
```{r warning=FALSE, eval=FALSE}
## merge dataset
data_tm1 <- left_join(dttm, data_tm1) %>% tbl_df %>% unique %>% arrange(index)
data_m1 <- left_join(dttm, data_m1) %>% tbl_df %>% unique %>% arrange(index)
...
# data_m1 %<>% ddply(.(index, week), na_locf) %>% tbl_df
#> data_m1 %>% anyNA
#[1] FALSE
...
saveRDS(data_m1, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_m1.rds')
saveRDS(data_tm1, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_tm1.rds')
```
...
- [比較 save/load, saveRDS/readRDS, feather, 與 data.table 套件的讀寫速度](http://steve-chen.tw/?p=555)
...
data_tm1 <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_tm1.rds')

data_tm1 %>% ddply(.(year, week), head, 1) %>% 
...
```{r echo=FALSE}
data_tm1 %>% ddply(.(year, week), tail, 1) %>% 
...
```{r warning=FALSE, eval=FALSE}
# --------- eval=FALSE ---------
#sq <- seq(1 , length(data_tm1$index), by = 1440)
#sets <- list()
timeID <- data_tm1$index %>% as_date %>% unique %>% sort
timeID %<>% .[. > ymd('2015-01-11')]

for (dt in timeID) {
  smp <- data_tm1 %>% tk_xts(silent = TRUE)
  dt %<>% as_date
  ...
  if(is.double(sets$forecast$index[1])){
    ...
    sets$forecast$index <- data_tm1$index[
    (which(data_tm1$index == smp %>% index %>% xts::last()) + 1):(
     which(data_tm1$index == smp %>% index %>% xts::last()) + 1440)]
  }
...
I set the length of dataset as monthly but the frequency set as 1440 minutes (per day).

```{r warning=FALSE, eval=FALSE}
# --------- eval=FALSE ---------
#sq <- seq(1 , length(data_tm1$index), by = 1440)
#sets <- list()
timeID <- data_tm1$index %>% as_date %>% unique %>% sort + days(1)
timeID %<>% .[. > ymd('2015-01-11')]
...
I set the length of dataset as quarterly but the frequency set as 1440 minutes (per day).
...
I set the length of dataset as yearly but the frequency set as 1440 minutes (per day).
...
```{r warning=FALSE, eval=FALSE}
ts_ <- data_tm1$close %>% ts()
mts_ <- data_tm1 %>% msts(seasonal.periods = c(1440, 7200), start = index(.)[1])
...
## http://manishbarnwal.com/blog/2017/05/03/time_series_and_forecasting_using_R/
smp %>% tk_ts(start = index(index)[1], frequency = c(1440, 7200)) %>% forecast %>% llply(tk_tbl)
...
for(i in (1:length(fs))) {
    ...
    if (dt %in% timeID) {
      ...
      sarima[[i]] <- tryCatch({llply(price_type, function(y) {
        ...
        df
      }) %>% tbl_df %>% bind_rows}, error = function(e) NULL)
...
fls <- paste0('data/fx/USDJPY/', list.files(
  'data/fx/USDJPY/', pattern = 'sets\\.[a-z]{2}\\.'))
readRDS(fls[1])

comp <- llply(fls, function(x) {
  if(grepl('sets.wk', x)) {
...
comp <- join(data_tm1, comp) %>% 
  tbl_df %>% na.omit
```

```{r warning=FALSE}
comp %>% ddply(.(Period), summarise, 
...
Below I recall the `ETS` models in previous paper for comparison.
...
ETS.MSE %>% 
  ...
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
  group_rows('AAN', 1, 25, label_row_css = 'background-color: #e68a00; color: #fff;') %>%
  group_rows('AAZ', 26, 50, label_row_css = 'background-color: #ff0000; color: #fff;') %>%
  group_rows('ANN', 51, 75, label_row_css = 'background-color: #bf80ff; color: #fff;') %>%
  group_rows('ANZ', 76, 100, label_row_css = 'background-color: #66ff33; color: #fff;') %>%
  group_rows('AZN', 101, 125, label_row_css = 'background-color: #6666ff; color: #fff;') %>%
  group_rows('AZZ', 126, 150, label_row_css = 'background-color: #66e0ff; color: #fff;') %>%
  group_rows('MAN', 151, 175, label_row_css = 'background-color:#0066ff; color: #fff;') %>%
  group_rows('MAZ', 176, 200, label_row_css = 'background-color: #ff9900; color: #fff;') %>%
  group_rows('MMN', 201, 225, label_row_css = 'background-color: #33ff33; color: #fff;') %>%
  group_rows('MMZ', 226, 250, label_row_css = 'background-color: #339966; color: #fff;') %>%
  group_rows('MNN', 251, 275, label_row_css = 'background-color: #5900b3; color: #fff;') %>%
  group_rows('MNZ', 276, 300, label_row_css = 'background-color: #269900; color: #fff;') %>%
  group_rows('MZN', 301, 325, label_row_css = 'background-color: #808000; color: #fff;') %>%
  group_rows('MZZ', 326, 350, label_row_css = 'background-color: #3399ff; color: #fff;') %>%
  group_rows('ZAN', 351, 375, label_row_css = 'background-color: #003380; color: #fff;') %>%
  group_rows('ZAZ', 376, 400, label_row_css = 'background-color: #804d00; color: #fff;') %>%
  group_rows('ZMN', 401, 425, label_row_css = 'background-color: #d279d2; color: #fff;') %>%
  group_rows('ZMZ', 426, 450, label_row_css = 'background-color: #666; color: #fff;') %>%
  group_rows('ZNN', 451, 475, label_row_css = 'background-color: #ff3377; color: #fff;') %>%
  group_rows('ZNZ', 476, 500, label_row_css = 'background-color: #993399; color: #fff;') %>%
  group_rows('ZZN', 501, 525, label_row_css = 'background-color: #00a3cc; color: #fff;') %>%
  group_rows('ZZZ', 526, 550, label_row_css = 'background-color: #e60000; color: #fff;') %>%
  scroll_box(width = '100%', height = '400px')
```
...
ETS.MSE %>% 
  ...
  kable(caption = 'MSE of daily Opened and Closed Transaction Orders') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
  group_rows('OpOp', 1, 22, label_row_css = 'background-color: #e68a00; color: #fff;') %>%
  group_rows('OpCl', 23, 44, label_row_css = 'background-color: #ff0000; color: #fff;') %>%
  group_rows('ANN', 45, 66, label_row_css = 'background-color: #bf80ff; color: #fff;') %>%
  group_rows('ANZ', 67, 88, label_row_css = 'background-color: #66ff33; color: #fff;') %>%
  group_rows('AZN', 89, 110, label_row_css = 'background-color: #6666ff; color: #fff;') %>%
  group_rows('AZZ', 111, 132, label_row_css = 'background-color: #66e0ff; color: #fff;') %>%
  group_rows('MAN', 133, 154, label_row_css = 'background-color:#0066ff; color: #fff;') %>%
  group_rows('MAZ', 155, 176, label_row_css = 'background-color: #ff9900; color: #fff;') %>%
  group_rows('MMN', 177, 198, label_row_css = 'background-color: #33ff33; color: #fff;') %>%
  group_rows('MMZ', 199, 220, label_row_css = 'background-color: #339966; color: #fff;') %>%
  group_rows('MNN', 221, 242, label_row_css = 'background-color: #5900b3; color: #fff;') %>%
  group_rows('MNZ', 243, 264, label_row_css = 'background-color: #269900; color: #fff;') %>%
  group_rows('MZN', 265, 286, label_row_css = 'background-color: #808000; color: #fff;') %>%
  group_rows('MZZ', 287, 308, label_row_css = 'background-color: #3399ff; color: #fff;') %>%
  group_rows('ZAN', 309, 330, label_row_css = 'background-color: #003380; color: #fff;') %>%
  group_rows('ZAZ', 331, 352, label_row_css = 'background-color: #804d00; color: #fff;') %>%
  group_rows('ZMN', 353, 374, label_row_css = 'background-color: #d279d2; color: #fff;') %>%
  group_rows('ZMZ', 375, 396, label_row_css = 'background-color: #666; color: #fff;') %>%
  group_rows('ZNN', 397, 418, label_row_css = 'background-color: #ff3377; color: #fff;') %>%
  group_rows('ZNZ', 419, 440, label_row_css = 'background-color: #993399; color: #fff;') %>%
  group_rows('ZZN', 441, 462, label_row_css = 'background-color: #00a3cc; color: #fff;') %>%
  group_rows('ZZZ', 463, 484, label_row_css = 'background-color: #e60cc0; color: #fff;') %>%
  group_rows('ZZN', 485, 506, label_row_css = 'background-color: #00a3cc; color: #fff;') %>%
  group_rows('ZZZ', 507, 528, label_row_css = 'background-color: #e56000; color: #fff;') %>%
  group_rows('ZZZ', 529, 550, label_row_css = 'background-color: #e33000; color: #fff;') %>%
  scroll_box(width = '100%', height = '400px')
```
...
ARIMA.MSE <- llply(ar.fls, function(x) {
    ...
    data.frame(Model = 'auto.arima', Type = nm, y) %>% tbl_df
  }) %>% bind_rows %>% 
  mutate(index = Date) %>% 
