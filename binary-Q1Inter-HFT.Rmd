...
for (dt in timeID) {
  smp <- data_tm1 %>% tk_xts(silent = TRUE)
  dt %<>% as_date
  smp <- smp[paste0(dt %m-% quarters(1) + seconds(59), '/', dt + seconds(59))]
...
```{r warning=FALSE, eval=FALSE}
# --------- eval=FALSE ---------
sq <- seq(1 , length(data_tm1$index), by = 7200)
sets <- list()
for(i in (1:length(fs))) {
    ...
    if (dt %in% timeID) {
      ...
      sets <- smp %>% tk_ts(start = index(index)[1], 
                            frequency = 7200) %>% 
        forecast(h=7200) %>% 
        llply(tk_tbl)
      
      wh <- which(data_tm1$index == smp %>% index %>% xts::last())
      sets$forecast$index = data_tm1$index[
        (which(data_tm1$index == smp %>% index %>% xts::last()) + 1):(
          which(data_tm1$index == smp %>% index %>% xts::last()) + 7200)]

      if (!dir.exists(paste0('data/fx/USDJPY'))) 
        dir.create(paste0('data/fx/USDJPY'))
      
      saveRDS(sets, paste0('data/fx/USDJPY/sets.7200.', 
                           as_date(sets$forecast$index[1]), '.rds'))
      
      cat(paste0(
        'data/fx/USDJPY/sets.7200.', 
        as_date(sets$forecast$index[1]), '.rds saved!\n'))
      }
  }
```
