...
pkgs <- c('devtools', 'knitr', 'kableExtra', 'tint', 
          ...
          'microbenchmark', 'dendextend', 'lhmetools', 'stringr', 'pacman')
...
Here I use `ts()` for seasonal data modeling and set `auto.arima(D = 1, seasonal = TRUE)` as **sarima** model, **sarimax** models will be modelled in next paper.
...
- [](Forecasting using R - Dynamic Regression (ch5).pdf)
...
for (dt in timeID) {
  ...
  sarimats <- smp %>% 
    ...
    #https://stackoverflow.com/a/37400899/3806250 #auto.arima(x, seasonal = T)
    auto.arima(D=1) %>% 
    forecast(h = 1440) %>% 
    llply(tk_tbl)
...
seasonal_m2 <- llply(fls2, function(x) {
  if(grepl('sets.wk', x)) {
...
seasonal_m2 <- join(data_m1, seasonal_m2) %>% 
  as_tibble %>% 
  na.omit
...
## don't use complete.cases() since there has some NA elements in ts models.
#seasonal_m <- seasonal_m[complete.cases(seasonal_m),]
write_rds(seasonal_m2, 'data/fx/USDJPY/seasonal_m2.rds')
#zip it to be splited 99MB for each file.
```

```{r warning=FALSE, message=FALSE}
seasonal_m2 <- read_rds('data/fx/USDJPY/seasonal_m2.rds')
```
...
mse2 %>% 
  ...
  kbl('html', caption = 'MSE of Seasonal Daily 1440 minutes ETS Model (Completed Period)', escape = F) %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
  kable_material() %>% 
  scroll_box(width = '100%')#, height = '400px')
```
...
ETS.MSE %>% mutate(
    ...
    MSE.2 = ifelse(
      ...
      cell_spec(
        paste0(round(MSE.2, 7), ' (rank: ', sprintf('%1.f', rank(MSE.2)), ')'), 
        'html', color = 'grey', italic = T))) %>% 
  kbl('html', caption = 'MSE of daily Opened and Closed Transaction Orders', escape = F) %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
