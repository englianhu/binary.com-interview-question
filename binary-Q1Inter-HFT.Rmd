...
pkgs <- c('knitr', 'kableExtra', 'tint', 'devtools', 'lubridate', 'data.table', 'quantmod', 'tidyquant', 'tidyr', 'timetk', 'plyr', 'stringr', 'magrittr', 'dplyr', 'tidyverse', 'memoise', 'htmltools', 'formattable', 'zoo', 'forecast', 'seasonal', 'seasonalview', 'rugarch', 'rmgarch', 'mfGARCH', 'sparklyr', 'microbenchmark', 'dendextend')
...
#Y2015W1_min1 %>% tk_ts(end = c(1440, 10080), frequency = 1440) %>% index %>% as.POSIXct(tz = 'UTC', origin = '1970-01-01')
...
#How to extract below index to datetime
#Y2015W1_min1 %>% msts(seasonal.periods=c(1440, 10080)) %>% .[,1] %>% as.numeric %>% as.POSIXct(tz = 'UTC', origin = '1970-01-01') %>% force_tz(tz = 'Europe/Athens')
Y2015W1_min1 %>% head %>% msts(seasonal.periods=c(1440, 10080))
```
...
# -------- eval=FALSE -----------
## weekly 1st day
dtm <- tsz2$index %>% date %>% .[seq(1, length(.), by = 2)]
#dtm <- seq(ymd_hms('2015-01-01 00:00:00'), ymd_hms('2017-08-31 00:00:00'), by = 60)

## create a seq of datetime to complete the  data point.
dttm <- llply(dtm, function(x) {
  x1 <- as.POSIXct(x) %>% 
      with_tz(tz = 'UTC') %>% 
      force_tz('Europe/Athens')
  
  x2 <- x1 + days(5)#days(4) + hours(23) + minutes(59)
  
  seq.POSIXt(from = x1, to = x2, by = '1 min')
  })

dttm %<>% data_frame(index = .) %>% unnest
```
...
Then read the datasets as we can know from above and below chunks.
...
```{r warning=FALSE, eval=FALSE}
data_tm1 <- left_join(dttm, data_tm1) %>% tbl_df %>% unique
data_m1 <- left_join(dttm, data_m1) %>% tbl_df %>% unique

saveRDS(data_m1, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_m1.rds')

saveRDS(data_tm1, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_tm1.rds')

data_comp <- merge(data_m1, data_tm1, all = TRUE) %>% 
  tbl_df %>% arrange(index)
data_comp <- data_comp[!duplicated(data_comp$index),]
data_comp <- llply(data_comp, na_locf) %>% bind_rows
```

```{r warning=FALSE, eval=FALSE}
#> data_tm1 %>% mice::md.pattern()
#        index AskOpen AskHigh AskLow AskClose BidOpen BidHigh BidLow BidClose       
#1257400     1       1       1      1        1       1       1      1        1      0
#60383       1       0       0      0        0       0       0      0        0      8
#            0   60383   60383  60383    60383   60383   60383  60383    60383 483064
#> data_m1 %>% mice::md.pattern()
#        index BidOpen BidHigh BidLow BidClose AskOpen AskHigh AskLow AskClose       
#1303354     1       1       1      1        1       1       1      1        1      0
#14429       1       0       0      0        0       0       0      0        0      8
#            0   14429   14429  14429    14429   14429   14429  14429    14429 115432

#https://www.rdocumentation.org/packages/dendextend/versions/1.8.0/topics/na_locf
data_comp <- join(data_tm1, data_m1) %>% tbl_df# %>% map(na_locf)
#> data_comp %>% mice::md.pattern()
#        index AskOpen AskHigh AskLow AskClose BidOpen BidHigh BidLow BidClose       
#1303354     1       1       1      1        1       1       1      1        1      0
#14429       1       0       0      0        0       0       0      0        0      8
#            0   14429   14429  14429    14429   14429   14429  14429    14429 115432

setDT(data_m1)[setDT(data_tm1), on = "index", `:=` 
               (AskOpen = i.AskOpen, AskHigh = i.AskHigh, AskLow = i.AskLow, AskClose = i.AskClose, 
                   BidOpen = i.BidOpen, BidHigh = i.BidHigh, BidLow = i.BidLow, BidClose = i.BidClose)]
```
...
```{r warning=FALSE}
rm(list = ls()[grep('Y2015W|tsz|tsz2|dc', ls())])
