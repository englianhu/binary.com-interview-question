...
pkgs <- c('devtools', 'knitr', 'kableExtra', 'tint', 'devtools', 'lubridate', 'data.table', 'feather', 'purrr', 'quantmod', 'tidyquant', 'tibbletime', 'furrr', 'flyingfox', 'tidyr', 'timetk', 'plyr', 'stringr', 'magrittr', 'dplyr', 'tidyverse', 'memoise', 'htmltools', 'formattable', 'zoo', 'forecast', 'seasonal', 'seasonalview', 'rugarch', 'rmgarch', 'mfGARCH', 'sparklyr', 'microbenchmark', 'dendextend')
...
[binary.com 面试试题 I - 单变量数据缺失值管理](http://rpubs.com/englianhu/handle-missing-value) and [binary.com面试试题 I - 单变量数据缺失值管理 II](http://rpubs.com/englianhu/428318) compares the missing values dataset and refill the missing value with `imputeTS::na.seadec()`: 
...
The paper concludes that the repaired dataset is workable since the MSE is very low. The paper compares tick-data-to-1min-data and also 1min-data where both datasets gather from FXCM.
...
Here I try to check if the filled dataset bias or not.
...
data_tm1 %>% mutate(
  ...
  bias.close = if_else(close>high|close<low, 1, 0)) %>% dplyr::filter(bias.open==1|bias.high==1|bias.low==1|bias.close==1) %>% 
...
comp %>% ddply(.(Period), summarise, 
               ...
               n = length(index)) %>% 
  tbl_df %>% 
  kable(caption = 'MSE of Seasonal ETS Model (Accumulated Period from Weekly)') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>%
  scroll_box(width = '100%')#, height = '400px')
```
...
comp2 %>% ddply(.(Period), summarise, 
               ...
               n = length(index)) %>% 
  tbl_df %>% 
  kable(caption = 'MSE of Seasonal ETS Model (Completed Period)') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>%
  scroll_box(width = '100%')#, height = '400px')
```
