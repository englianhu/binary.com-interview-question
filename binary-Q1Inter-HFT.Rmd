...
fit_ts <- auto.arima(Op(sarimamsts), D = 1, trace = TRUE)
...
mse1 %>% 
  ...
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
  kable_material() %>% 
  scroll_box(width = '200%')#, height = '400px')
```
  ...
  mutate(
    ...
    open.rmse = ifelse(
      ...
      cell_spec(
        paste0(round(open.rmse, 7), ' (rank: ', sprintf('%1.f', rank(open.rmse)), ')'), 
        'html', color = 'grey', italic = T))) %>% 
  kbl('html', caption = 'Comparison of Models (1 min summarised to 1 day per unit)', escape = F) %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
  kable_material() %>% 
  ...
  mutate(
    ...
    close.rmse = ifelse(
      ...
      cell_spec(
        paste0(round(close.rmse, 7), ' (rank: ', sprintf('%1.f', rank(close.rmse)), ')'), 
        'html', color = 'grey', italic = T))) %>% 
  kbl('html', caption = 'Comparison of Models', escape = F) %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>% 
  kable_material() %>% 
  scroll_box(width = '500%')#, height = '400px')
```
...
fit_ts <- readRDS('data/fx/USDJPY/sarima_msts_sample.rds')

##Below model use dataset where contain 7200 mins and forecast nested 60mins & 1440 mins.
fit_msts_60_1440 <- readRDS(fit_msts_60_1440, 'data/fx/USDJPY/sarima_msts_60_1440_sample.rds')
```
