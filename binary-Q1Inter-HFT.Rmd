...
subtitle: "[Deriv.com](https://github.com/englianhu/binary.com-interview-question) Interview Question I - Interday High Frequency Trading Models Comparison"
author: "[®γσ, Lian Hu](https://englianhu.github.io/) <img src='www/Me byteDance.jpg' width='24'> <img src='www/RYU.jpg' width='24'> <img src='www/ENG.jpg' width='24'>®"
...
There has already 2 years passed by while [Binary.com-is-Rebranding-to-Deriv.com](https://derivdotcom.medium.com/binary-com-is-rebranding-to-deriv-com-and-here-is-everything-you-need-to-know-6f4a8513c84b), Here I sumarised some previous reserahc papers in [Binary.com → Deriv.com](https://englianhu.medium.com/binary-com-deriv-com-6058cdbfc3a1).
...
> readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_m1.rds') %>% mutate(
...
> readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_tm1.rds') %>% mutate(
+     open = BidOpen, high = BidHigh, low = BidLow, close = BidClose, 
+     bias.open = if_else(open>high|open<low, 1, 0), 
+     bias.high = if_else(high<open|high<low|high<close, 1, 0), 
+     bias.low = if_else(low>open|low>high|low>close, 1, 0), 
...
## Shows example.
readRDS(grep('sets', fls, value = TRUE)[1])
readRDS(grep('mts', fls, value = TRUE)[1]) %>% tbl_df
...
comp <- join(data_m1, comp) %>% 
  tbl_df %>% 
  na.omit
```

```{r warning=FALSE, message=FALSE}
comp %>% ddply(.(Model, Period), summarise, 
               ...
               n = length(index)) %>% 
  tbl_df %>% 
  kable(caption = 'MSE of Seasonal Daily 1440 minutes ETS Model (Accumulated Period from Weekly)') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>%
  scroll_box(width = '100%')#, height = '400px')
```
...
comp %>% 
  ...
  ddply(.(Model, Period), summarise, 
        ...
        n = length(index)) %>% 
  tbl_df %>% 
  kable(caption = 'MSE of Seasonal Daily 1440 minutes ETS Model (Completed Period)') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>%
  scroll_box(width = '100%')#, height = '400px')
```
...
## Shows example.
readRDS(grepl('sets', fls, value = TRUE)[1])
readRDS(grepl('mts', fls, value = TRUE)[1])

comp <- llply(fls, function(x) {
  if(grepl('sets.wk', x)) {
    dat <- readRDS(x)$forecast %>% mutate(Period = factor('wk'))
    
  } else if(grepl('sets.mo', x)) {
    dat <- readRDS(x)$forecast %>% mutate(Period = factor('mo'))
    
  } else if(grepl('sets.qt', x)) {
    dat <- readRDS(x)$forecast %>% mutate(Period = factor('qt'))
    
  }else if(grepl('sets.yr', x)) {
    dat <- readRDS(x)$forecast %>% mutate(Period = factor('yr'))
    
  } else if(grepl('mts.dy.wk.[0-9]{4}', x)) {
    dat <- readRDS(x) %>% mutate(Period = factor('dy.wk'))
    
  } else if(grepl('mts.dy.wk.mo.[0-9]{4}', x)) {
    dat <- readRDS(x) %>% mutate(Period = factor('dy.wk.mo'))
    
  } else if(grepl('mts.dy.wk.mo.qt.[0-9]{4}', x)) {
    dat <- readRDS(x) %>% mutate(Period = factor('dy.wk.mo.qt'))
    
  } else if(grepl('mts.dy.wk.mo.yr.[0-9]{4}', x)) {
    dat <- readRDS(x) %>% mutate(Period = factor('dy.wk.mo.yr'))
