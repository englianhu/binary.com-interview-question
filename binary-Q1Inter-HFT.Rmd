...
author: "[®γσ, Lian Hu](https://englianhu.github.io/) <img src='www/Me byteDance.jpg' width='24'> <img src='www/RYU.jpg' width='24'> <img src='www/ENG.jpg' width='28'>® <img src='www/xueba.jpg' width='28'>"
...
options(warn = -1, encoding = 'native')#, digits.secs = 6)
rm(pkgs, funs)
```
...
data_m1 %>% ddply(.(year, week), head, 1) %>% 
  ...
  scroll_box(width = '100%', height = '400px')
```
By refered to sportsbook experience, here I plot above table which shows weekly initial price.
...
data_m1 %>% ddply(.(year, week), tail, 1) %>% 
  ...
  scroll_box(width = '100%', height = '400px')
```
By refered to sportsbook experience again, here I plot above table which shows weekly closed price.
...
comp <- join(data_m1, comp) %>% ##The last observation of Friday is Saturday 00:00:00 but not next Monday 00:00:00. Therefore the Saturday forecasted price will not be merge with Next Monday market price. The 
  as_tibble %>% 
  na.omit
```
