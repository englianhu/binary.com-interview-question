...
#dtm <- seq(from = ymd('2015-01-05'), to = ymd('2018-07-10'), by = 'weeks')
dtm <- data_tm1 %>% 
...
dttm <- llply(1:nrow(dtm), function(i) {
  ...
  res <- seq.POSIXt(from = x1, to = x2, by = '1 min')
  data_frame(index = res[-1]) %>% cbind(., dtm[i,-1])
  }) %>% bind_rows %>% tbl_df
```
...
## https://gist.github.com/christophsax/3db87a48596768b232a26dfce87c3299
## http://steve-chen.tw/?p=555
...
```{r warning=FALSE, eval=FALSE}
data_m1 <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_m1.rds')
data_tm1 <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_tm1.rds')

data_m1_fill <- merge(data_m1, data_tm1, all = TRUE) %>% 
  tbl_df %>% arrange(index)
data_m1_fill <- data_m1_fill[!duplicated(data_m1_fill$index),]
rm(data_m1, data_tm1)
...
# data_m1_fill %>% split(data_m1_fill$index) %>% 
#   llply(function(x) {
#     na.locf(na.locf(x), fromLast = TRUE)
#   }) %>% do.call(rbind, .)

# data_m1_fill %<>% ddply(.(index, week), na_locf) %>% tbl_df
#> data_m1_fill %>% anyNA
#[1] FALSE

## Time consuming and didn't fill up the NA columns.
#data_m1_fill %<>% 
#  group_by(index, week) %>% 
...
data_m1_fill %<>% 
  ddply(.(year, week), function(x) {
       x %>% fill(BidOpen, BidHigh, BidLow, BidClose, 
                  AskOpen, AskHigh, AskLow, AskClose) %>% #default direction down
             fill(BidOpen, BidHigh, BidLow, BidClose, 
                  AskOpen, AskHigh, AskLow, AskClose, .direction = 'up')
    }) %>% tbl_df
...
saveRDS(data_m1_fill, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_m1_fill.rds')
```
...
Finally, I filled-up the NA section by `data_tm1` and eventually filled up by `na_locf` function.
...
data_m1_fill <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/data_m1_fill.rds')

data_m1_fill %>% ddply(.(year, week), head, 1) %>% 
