...
pkgs <- c('knitr', 'kableExtra', 'tint', 'devtools', 'lubridate', 'data.table', 'quantmod', 'tidyquant', 'timetk', 'plyr', 'stringr', 'magrittr', 'dplyr', 'tidyverse', 'memoise', 'htmltools', 'formattable', 'zoo', 'forecast', 'seasonal', 'seasonalview', 'rugarch', 'rmgarch', 'mfGARCH', 'sparklyr', 'microbenchmark')
...
Y2015W1_min1 <- Y2015W1 %>% 
  convertOHLC(combine = TRUE, mean = TRUE, .unit = t.unit[2]) %>% 
...
Kindly refer to section [Seasonal Data] or [Seasonal periods](https://robjhyndman.com/hyndsight/seasonal-periods/) to know the seasonality dataset.
...
#Y2015W1 %>% tk_ts(end = c(1440, 10080), frequency = 1440) %>% attributes %>% .$index %>% as.POSIXct(tz = 'UTC', origin = '1970-01-01') %>% force_tz(tz = 'Europe/Athens')
Y2015W1_min1 %>% tk_ts(end = c(3, 10080), frequency = 1440) %>% .[,1:ncol(.)] %>% head
...
Due to the `UTC` timezone has no daylight saving issue, therefore the initial trading time will be a problem where I need to cope with.
...
llply(fls[[7]], function(x) {
    ...
    mbase %<>% mutate(
      index = ymd_hm(substr(index, 1, nchar(index) + 6)), 
      index = format(index, tz='Europe/Athens',usetz=TRUE) %>% 
        as.POSIXct(tz='Europe/Athens'))
    ...
    #mbase %<>% msts(seasonal.periods = c(1440, 10080))
...
From above tables, we can know the daylight saving detection and `datetime` auto convertion is not tally. Here I united all intial observation started from `00:00:00`.

## Completion of Data

### Data Selection

[binary.com 面试试题 I - 数据缺失值管理](http://rpubs.com/englianhu/handle-missing-value) compares the missing values dataset and refill the missing value, the paper concludes that the repaired dataset is workable since the MSE is very low. The paper compares tick-data-to-1min-data and also 1min-data where both datasets gather from FXCM.

```{r, warning=FALSE}
## Execute below codes and save it to save the time.
## read tick-data and convert to 1 min dataset.
# dc <- llply(fls[[7]], function(x) {
#     y <- readRDS(x) %>% convertOHLC(combine = TRUE, mean = TRUE, .unit = t.unit[2])
#     y %>% dplyr::count(index)
# })
# 
# saveRDS(dc, 'C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/count_tick_data_1min.rds')
dc <- readRDS('C:/Users/scibr/Documents/GitHub/scibrokes/real-time-fxcm/data/USDJPY/count_tick_data_1min.rds')
dc %>% ldply(nrow) %>% tbl_df
dc %>% ldply(nrow) %>% tbl_df %>% 
  dplyr::filter(V1 < 6000)
dc %>% ldply(nrow) %>% tbl_df %>% 
  dplyr::count(V1) %>% 
  kable(caption = 'Count data point') %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'condensed', 'responsive')) %>%
...
Above tables show the dataset is incompleted.
