...
if(!exists('seasonal_m1')) {
  seasonal_m1 <- read_rds('data/fx/USDJPY/seasonal_m1.rds')}
ftl1 <- filter(seasonal_m1, index <= as_date('2017-12-31'))
ftl1 %<>% data.table()
#setkey(ftl1)
...
ftl_dat1 <- join_all(list(m.op, m.hi, m.lo, m.cl)) %>% 
  as_tibble
```
...
ftl_dat2 <- join_all(list(m.op, m.hi, m.lo, m.cl)) %>% 
  as_tibble
```
...
ftl_dat3 <- join_all(list(m.op, m.hi, m.lo, m.cl)) %>% 
  as_tibble
```
